...
INTERN __NOCONNECT KERNEL_SELECT(size_t, ssize_t) CC
...
again_connect:
...
#else /* __KERNEL__ */
	{
		...
		if (lfutexlockexpr(&self->rb_nempty, self, expr, COMPILER_LENOF(expr), NULL, 0) < 0)
			return -1;
	}
#endif /* !__KERNEL__ */
	/* Try to read more data. */
...
again_connect:
...
#else /* __KERNEL__ */
	{
		...
		if (size >= limit) {
			...
			if (lfutexlockexpr(&self->rb_nfull, self, expr, COMPILER_LENOF(expr), NULL, 0) < 0)
				return -1;
		}
	}
#endif /* !__KERNEL__ */
...
INTERN KERNEL_SELECT(size_t, ssize_t) CC
