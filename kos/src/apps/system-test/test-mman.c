...
DEFINE_TEST(mlock_and_mincore) {
	...
	*(unsigned int *)p = 0x1234;
	...
	assertf(mincore(p, ps, incore) == 0, "mincore() failed: %s:%d", strerror(errno), errno);
	assertf(incore[0] == 0, "incore[0] = %#" PRIx8, incore[0]);
	assertf(mlock2(p, ps, MLOCK_ONFAULT) == 0, "mlock() failed: %s:%d", strerror(errno), errno);
	incore[0] = 0xcc;
	assertf(mincore(p, ps, incore) == 0, "mincore() failed: %s:%d", strerror(errno), errno);
	assertf(incore[0] == 0, "incore[0] = %#" PRIx8, incore[0]);
	COMPILER_WRITE_BARRIER();
	*(unsigned int *)p = 0x1234;
	...
	assertf(mincore(p, ps, incore) == 0, "mincore() failed: %s:%d", strerror(errno), errno);
	assertf(incore[0] == 0, "incore[0] = %#" PRIx8, incore[0]);
	assertf(mlock2(p, ps, 0) == 0, "mlock() failed: %s:%d", strerror(errno), errno);
	incore[0] = 0xcc;
	assertf(mincore(p, ps, incore) == 0, "mincore() failed: %s:%d", strerror(errno), errno);
	assertf(incore[0] == 1, "incore[0] = %#" PRIx8, incore[0]);
	/* Call `munlock()' just so we've called it at least once! */
	assertf(munlock(p, ps) == 0, "munlock failed: %s:%d", strerror(errno), errno);
	assertf(munmap(p, ps) == 0, "munmap failed: %s:%d", strerror(errno), errno);
}
...
DEFINE_TEST(mremap) {
	...
	*(unsigned int *)p = 0x1234;
	...
	assertf(p == mremap(p, ps * 2, ps, MREMAP_MAYMOVE), "mremap() failed: %s:%d", strerror(errno), errno);
