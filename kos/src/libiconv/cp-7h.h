...
struct iconv_7h_codepage {
	...
	char16_t i7cp_decode[128];

	/* Sorted by `icee_uni': table for how to encode UNI into the cp. */
	COMPILER_FLEXIBLE_ARRAY(struct iconv_codepage_encode_entry, i7cp_encode);
#define i7cp_encode_max i7cp_encode[0]._icee_pad /* # of encode entries. */
};
