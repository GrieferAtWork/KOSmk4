 ...
 */
#ifndef GUARD_MODFAT_OLDFATFAT_C
#define GUARD_MODFAT_OLDFATFAT_C 1
...
#define VERIFY_INODE_FSDATA(node)                                    \
	assert((node) == (node)->i_super                                 \
	       ? (node)->i_fsdata == &((FatSuperblock *)(node))->ft_root \
	       : kmalloc_usable_size((node)->i_fsdata) >= sizeof(FatNodeData))
#endif
...
Fat16_VReadFromRootDirectory(FatSuperblock *__restrict self,
	...
	assert(self->i_fsdata == &self->ft_root);
	max_read = self->ft_root.fn16_root.r16_rootsiz;
	...
	block_device_read(self->s_device,
	                  buf,
	                  max_read,
	                  self->ft_root.fn16_root.r16_rootpos + pos);
}
...
Fat16_VWriteToRootDirectory(FatSuperblock *__restrict self,
	...
	assert(self->ft_type != FAT32);
	assert(self->i_fsdata == &self->ft_root);
	max_write = self->ft_root.fn16_root.r16_rootsiz;
...
Fat_VReadFromINode(struct inode *__restrict self,
	...
	if (self != self->i_super ||
	    ((FatSuperblock *)self)->ft_type == FAT32) {
...
Fat_GetAbsDiskPos(struct inode *__restrict self, pos_t pos)
		THROWS(...) {
	if (self != self->i_super ||
	    ((FatSuperblock *)self->i_super)->ft_type == FAT32) {
		FatSuperblock *fat = (FatSuperblock *)self->i_super;
	...
	pos += self->i_fsdata->fn16_root.r16_rootpos;
	return pos;
}
...
preallocate_cluster_vector_entries(struct inode *__restrict node,
	...
	new_alloc = CEILDIV((size_t)node->i_filesize, fat->ft_clustersize);
	if (new_alloc < min_num_clusters)
		new_alloc = min_num_clusters;
	if unlikely(new_alloc <= data->fn_clustera)
		return; /* nothing to do here... */
	if unlikely(!new_alloc)
		new_alloc = 2;
	TRY {
		data->fn_clusterv = (FatClusterIndex *)krealloc(data->fn_clusterv,
...
allocate_smallest_possible:
		...
		if (min_num_clusters <= data->fn_clustera)
			return;
		new_alloc = min_num_clusters;
 ...
 * chain for) the `nth_cluster' of the given `self' */
...
Fat_GetFileCluster(struct inode *__restrict self,
	...
	if unlikely(!data->fn_clusterc) {
		/* Load the initial file cluster. */
		inode_loadattr(self);
		assert(data->fn_clusterc != 0);
	}
	if (nth_cluster < data->fn_clusterc) {
		result = data->fn_clusterv[nth_cluster];
		if (result < fat->ft_cluster_eof)
			return result; /* Simple case: cluster already cached. */
		assert(nth_cluster == data->fn_clusterc - 1);
	} else {
		result = data->fn_clusterv[data->fn_clusterc - 1];
		if (result < fat->ft_cluster_eof) {
			/* Not all clusters have been loaded, yet. -> load more now. */
			sync_write(self);
			TRY {
				preallocate_cluster_vector_entries(self, data, fat, nth_cluster + 1);
				for (;;) {
					result = Fat_GetFatIndirection(fat, result);
					data->fn_clusterv[data->fn_clusterc] = result;
					++data->fn_clusterc;
					if (result >= fat->ft_cluster_eof) {
						...
						new_vector = (FatClusterIndex *)krealloc_nx(data->fn_clusterv,
						                                            data->fn_clusterc * sizeof(FatClusterIndex),
						                                            FS_GFP);
						if likely(new_vector)
							data->fn_clusterv = new_vector;
						sync_endwrite(self);
						return result;
					}
					if (nth_cluster < data->fn_clusterc) {
						...
						assert(nth_cluster == data->fn_clusterc - 1);
						assert(result == data->fn_clusterv[nth_cluster]);
						break;
					}
				}
			} EXCEPT {
	...
	assert(data->fn_clusterv[data->fn_clusterc - 1] >= fat->ft_cluster_eof);
	if (!(mode & FAT_GETCLUSTER_MODE_FCREATE))
		return fat->ft_cluster_eof_marker; /* EOF */
again_lock_node:
	sync_write(self);
	TRY {
		preallocate_cluster_vector_entries(self, data, fat, nth_cluster + 1);
...
create_more_clusters_already_locked:
	if unlikely(!sync_trywrite(&fat->ft_fat_lock)) {
		sync_endwrite(self);
		sync_write(&fat->ft_fat_lock);
		sync_endwrite(&fat->ft_fat_lock);
		goto again_lock_node;
	}
	TRY {
		assert(data->fn_clusterv[data->fn_clusterc - 1] >= fat->ft_cluster_eof);
		assert(data->fn_clustera >= nth_cluster + 1);
		/* Create more clusters. */
		for (;;) {
			result = Fat_FindFreeCluster(fat);
			/* Mark the new cluster as allocated. */
			Fat_SetFatIndirection(fat, result,
			                      fat->ft_cluster_eof_marker);
			TRY {
				if (!(mode & FAT_GETCLUSTER_MODE_FNOZERO))
					zero_initialize_cluster(fat, result);
				data->fn_clusterv[data->fn_clusterc - 1] = result;
				data->fn_clusterv[data->fn_clusterc] = fat->ft_cluster_eof_marker;
				++data->fn_clusterc;
				TRY {
					if (data->fn_clusterc == 2) {
						...
						assert(data->fn_clusterc >= 3);
						/* -3: PREV_CLUSTER */
						/* -2: THIS_CLUSTER */
						/* -1: fat->ft_cluster_eof_marker */
						Fat_SetFatIndirection(fat,
						                      data->fn_clusterv[data->fn_clusterc - 3],
						                      result);
					}
				} EXCEPT {
					--data->fn_clusterc;
					assert(data->fn_clusterv[data->fn_clusterc - 1] == result);
					data->fn_clusterv[data->fn_clusterc - 1] = fat->ft_cluster_eof_marker;
					RETHROW();
				}
			} EXCEPT {
			...
			if (nth_cluster < data->fn_clusterc - 1) {
				/* The requested cluster was reached */
				assert(nth_cluster == data->fn_clusterc - 2);
				assert(result == data->fn_clusterv[nth_cluster]);
				break;
			}
		}
	...
	assert(result == data->fn_clusterv[nth_cluster]);
	assert(result < fat->ft_cluster_eof);
#ifndef NDEBUG
	{
		size_t i;
		assert(data->fn_clusterv[data->fn_clusterc - 1] >= fat->ft_cluster_eof);
		for (i = 0; i < data->fn_clusterc - 1; ++i) {
			FatClusterIndex curr, next, real_next;
			curr      = data->fn_clusterv[i];
			next      = data->fn_clusterv[i + 1];
...
FatDirectory_AllocateFreeRange(struct directory_node *__restrict self,
	...
	for (i = 0;;) {
		if (Fat_VTryReadFromINode(self, (byte_t *)&entry, sizeof(entry),
		                          (pos_t)i * sizeof(struct fat_dirent)) < sizeof(struct fat_dirent) ||
...
typedef struct ATTR_PACKED {
	u8      ffs_align;
	struct fat_dirent ffs_file;  /* The LFN characters are unaligned, so we manually align them here */
} FatFileStorage;
...
Fat_ReadDirectory(struct directory_node *__restrict self,
	...
	struct fat_dirent &fatfile = filestorage.ffs_file;
#else
...
continue_reading:
	...
	if (fatfile.f_marker == MARKER_UNUSED) {
		...
		if ((pos % sizeof(struct fat_dirent)) == 0)
			FatDirectory_AddFreeFile(self, (u32)(pos / sizeof(struct fat_dirent)));
...
#define FILE_IS_LFN(x)                      \
	...
	if (FILE_IS_LFN(fatfile)) {
		...
		u32 lfn_valid   = 0;
		pos_t lfn_start = pos - sizeof(struct fat_dirent);
		do {
			...
			for (;;) {
				...
				if (fatfile.f_marker != MARKER_UNUSED)
					break;
				if ((pos % sizeof(struct fat_dirent)) == 0)
					FatDirectory_AddFreeFile(self, (u32)(pos / sizeof(struct fat_dirent)) - 1);
			}
		...
		/* Merge LFN filename segments. */
		{
			...
			result->de_fsdata.de_start = pos - sizeof(struct fat_dirent);
			/* Use the absolute-ondisk position of the file's struct fat_dirent as INode number. */
...
dos_8dot3:
		...
		*dst        = 0;
		...
		if (name_length <= 2) {
			if unlikely(!name_length) {
				printk(KERN_ERR "[fat] Unnamed directory entry at "
				       ...
				       self->i_fileino, pos - sizeof(struct fat_dirent));
		...
		result->de_pos             = pos - sizeof(struct fat_dirent);
...
NOTHROW(KCALL Fat_FinalizeNode)(struct inode *__restrict self) {
	VERIFY_INODE_FSDATA(self);
	if (self->i_fsdata) {
		kfree(self->i_fsdata->fn_clusterv);
		kfree(self->i_fsdata);
	}
}
...
Fat_DecodeFileDate(struct fat_filedate self)
		THROWS(...) {
	time_t result;
	unsigned int year;
...
LOCAL ATTR_CONST struct fat_filedate
NOTHROW(KCALL Fat_EncodeFileDate)(time_t tmt) {
	struct fat_filedate result;
	unsigned int year;
	u8 i;
...
NOTHROW(KCALL Fat_DecodeFileTime)(struct fat_filetime self) {
	return ((time_t)self.ft_hour * 60 * 60) +
	       ((time_t)self.ft_min * 60) +
	       ((time_t)self.ft_sec * 2);
}

LOCAL ATTR_CONST struct fat_filetime
NOTHROW(KCALL Fat_EncodeFileTime)(time_t tmt) {
	struct fat_filetime result;
...
NOTHROW(KCALL Fat_MaskINodeAttributes)(struct inode *__restrict self) {
	FatSuperblock *super = (FatSuperblock *)self->i_super;
	{
		struct fat_filetime temp_time;
		struct fat_filedate temp_date;
		/* Truncate timestamps. */
	...
	if (!(super->ft_features & FAT_FEATURE_ARB)) {
		...
		self->i_filemode |= super->ft_mode & 0555;
	...
	if (super->ft_features & FAT_FEATURE_UGID) {
		...
		struct fat_filedate temp;
		/* Without uid/gid support, all files use a filesystem-global uid/gid pair */
		self->i_fileuid = super->ft_uid;
		self->i_filegid = super->ft_gid;
...
Fat_LoadINodeFromFatFile(struct inode *__restrict self,
                         struct fat_dirent const *__restrict file)
		THROWS(...) {
	FatNodeData *data;
	FatSuperblock *super;
	VERIFY_INODE_FSDATA(self);
	...
	/* file --> self */
	assert((data->fn_clusterv != NULL) ==
	       (data->fn_clustera != 0));
	if (!data->fn_clustera) {
		data->fn_clusterv = (FatClusterIndex *)kmalloc(2 * sizeof(FatClusterIndex),
		                                              FS_GFP);
		data->fn_clustera = 2;
	}
	/* Setup the initial cluster. */
	data->fn_clusterc    = 1;
	data->fn_clusterv[0] = LETOH16(file->f_clusterlo);
	memcpy(&data->fn_file, &file->f_attr, sizeof(data->fn_file));
	...
	if (super->ft_features & FAT_FEATURE_ARB) {
		...
		data->fn_file.ff_arb = arb;
	} else {
		assert(!(super->ft_mode & ~0777));
		/* 32-bit clusters */
		data->fn_clusterv[0] |= (u32)LETOH16(file->f_clusterhi) << 16;
...
NOTHROW(KCALL Fat_SaveINodeToFatFile)(struct inode const *__restrict self,
                                      struct fat_dirent *__restrict file) {
	FatSuperblock *super;
	FatNodeData *data;
	u32 cluster;
	VERIFY_INODE_FSDATA(self);
	...
	assert((data->fn_clusterv != NULL) ==
	       (data->fn_clustera != 0));
	if (!data->fn_clusterc) {
		cluster = ((FatSuperblock *)self->i_super)->ft_cluster_eof_marker;
	} else {
		cluster = data->fn_clusterv[0];
	}
	/* Copy basic file data. */
	memcpy(&file->f_attr, &data->fn_file, sizeof(data->fn_file));
	file->f_clusterlo = HTOLE16(cluster & 0xffff);
	if (super->ft_features & FAT_FEATURE_ARB) {
		u16 arb = data->fn_file.ff_arb;
		...
		if (!(unix_perm & 0222)) {
			...
			arb |= data->fn_file.ff_arb & (FAT_ARB_NO_OW | FAT_ARB_NO_GW | FAT_ARB_NO_WW);
		}
...
Fat_LoadINodeAttributes(struct inode *__restrict self)
		THROWS(E_IOERROR, ...) {
	byte_t buf[sizeof(struct fat_dirent) - offsetof(struct fat_dirent, f_attr)];
	/* Read the FAT-FILE from disk. */
	block_device_read(self->i_super->s_device, buf, sizeof(buf),
	                  (pos_t)self->i_fileino + offsetof(struct fat_dirent, f_attr));
	/* Load Node data from the file. */
	Fat_LoadINodeFromFatFile(self, container_of((u8 *)buf, struct fat_dirent, f_attr));
}
...
Fat_SaveINodeAttributes(struct inode *__restrict self)
	...
	Fat_SaveINodeToFatFile(self, container_of((u8 *)buf, struct fat_dirent, f_attr));
...
Fat_Ioctl(struct inode *__restrict self, syscall_ulong_t cmd,
	...
	case FAT_IOCTL_GET_ATTRIBUTES:
		...
		require(CAP_FOWNER);
		*(USER CHECKED u32 *)arg = self->i_fsdata->fn_file.ff_attr;
		break;
	...
	case FAT_IOCTL_GET_VOLUME_ID: {
		...
		*(USER CHECKED u32 *)arg = super->ft_volid;
	}	break;
...
Fat_DoTruncateINode(struct inode *__restrict self,
	...
	delete_start = Fat_GetFileCluster(self, new_cluster_count,
	                                  FAT_GETCLUSTER_MODE_FNORMAL);
	if (delete_start >= fat->ft_cluster_eof)
	...
	node->fn_clusterv[new_cluster_count] = fat->ft_cluster_eof_marker;
	node->fn_clusterc                    = new_cluster_count + 1;
}
...
Fat_TruncateINode(struct inode *__restrict self, pos_t new_size)
	...
	Fat_DoTruncateINode(self,
	                    (size_t)CEILDIV(new_size,
	                                    ((FatSuperblock *)self->i_super)->ft_clustersize));
}
...
Fat_UnlinkFileFromParentDirectoryImpl(struct directory_node *__restrict containing_directory,
	...
	for (i = min; i <= max; i += sizeof(struct fat_dirent)) {
	...
	FatDirectory_AddFreeRange(containing_directory,
	                          (u32)(min / sizeof(struct fat_dirent)),
	                          (u32)(((max - min) / sizeof(struct fat_dirent)) + 1));
...
PRIVATE NOBLOCK ATTR_RETNONNULL WUNUSED NONNULL((1, 2, 3, 4)) struct fat_dirent *KCALL
Fat_GenerateFileEntries(struct fat_dirent *__restrict buffer,
	...
	}
	{
		...
		if (req_files > avail_files) {
			...
			buffer = inhert_heap_buffer
			         ? (struct fat_dirent *)krealloc(buffer, req_files * sizeof(struct fat_dirent), FS_GFP)
			         : (struct fat_dirent *)kmalloc(req_files * sizeof(struct fat_dirent), FS_GFP);
		}
		*pbuffer_length = req_files;
		...
		for (i = 0; i < req_files; ++i) {
			struct fat_dirent *entry = &buffer[i];
		...
		memcpy(&buffer[req_files], &dos83, sizeof(struct fat_dirent));
	}
	return buffer;
}
...
(KCALL Fat_GenerateFileEntriesAuto)(struct directory_node *__restrict target_directory,
                                    struct fat_dirent *__restrict buffer,
                                    u32 *__restrict pbuffer_length,
                                    struct fdirent *__restrict target_dirent,
                                    struct inode *__restrict new_node)
		THROWS(E_BADALLOC) {
	struct fat_dirent *result  = buffer;
	uintptr_t cookie = 0;
again:
...
PRIVATE struct fat_dirent const sential_fatfile = { 0 };
...
Fat_AddFileToDirectory(struct directory_node *__restrict target_directory,
		...
		THROWS(E_FSERROR_UNSUPPORTED_OPERATION, E_FSERROR_ILLEGAL_PATH,
		       ...
		       E_IOERROR, ...) {
	FatSuperblock *super;
	struct fat_dirent buffer[4], *files;
	...
	TRY {
		...
		assert(data);
		first_cluster = data->fn_clusterc != 0
		                ? data->fn_clusterv[0]
		...
		target_index = FatDirectory_AllocateFreeRange(target_directory,
		                                              file_count,
		                                              &is_directory_end);
		target_position = (pos_t)target_index * sizeof(struct fat_dirent);
		TRY {
			...
			if (is_directory_end) {
				...
				Fat_VWriteToINode(target_directory,
				                  ...
				                  target_position + file_count * sizeof(struct fat_dirent));
			}
			Fat_VWriteToINode(target_directory,
			                  (byte_t *)files,
			                  file_count * sizeof(struct fat_dirent),
			                  target_position);
		} EXCEPT {
			if (!is_directory_end)
		...
		target_dirent->de_fsdata.de_start = (pos_t)target_position + (file_count - 1) * sizeof(struct fat_dirent);
		/* Fill in INode information for the location of the struct fat_dirent */
		...
		data->fn_file.ff_attr    = files[file_count - 1].f_attr;
		data->fn_file.ff_ntflags = files[file_count - 1].f_ntflags;
		if (super->ft_features & FAT_FEATURE_ARB)
			data->fn_file.ff_arb = LETOH16(files[file_count - 1].f_arb);
	} EXCEPT {
		if (files != buffer)
			kfree(files);
		RETHROW();
...
Fat_CreateFileInDirectory(struct directory_node *__restrict target_directory,
	...
	node = (FatNodeData *)kmalloc(sizeof(FatNodeData),
	                                    FS_GFP | GFP_CALLOC);
	TRY {
		new_node->i_fsdata  = node;
		node->fn_clusterv    = (FatClusterIndex *)kmalloc(2 * sizeof(FatClusterIndex), FS_GFP);
		node->fn_clustera    = 2;
		node->fn_clusterc    = 1;
		node->fn_clusterv[0] = super->ft_cluster_eof_marker;
		/* Add the file to its parent's directory. */
		Fat_AddFileToDirectory(target_directory, target_dirent, new_node);
	} EXCEPT {
		kfree(new_node->i_fsdata->fn_clusterv);
...
Fat_CreateDirectoryInDirectory(struct directory_node *__restrict target_directory,
	...
	TRY {
		...
		struct fat_dirent pattern[COMPILER_LENOF(new_directory_pattern)];
		new_node->i_fsdata  = node;
		node->fn_clusterv    = (FatClusterIndex *)kmalloc(2 * sizeof(FatClusterIndex), FS_GFP);
		node->fn_clustera    = 2;
		node->fn_clusterc    = 1;
		...
		if (super->ft_type == FAT32 || target_directory != super) {
		...
		TRY {
			...
			Fat_DeleteClusterChain((FatSuperblock *)target_directory->i_super,
			                       new_node->i_fsdata->fn_clusterv[0]);
			RETHROW();
		}
	} EXCEPT {
		kfree(new_node->i_fsdata->fn_clusterv);
...
Fat_CygwinSymlinkCreate(struct directory_node *__restrict target_directory,
	...
	super = (FatSuperblock *)target_directory->i_super;
	if unlikely(super->ft_features & FAT_FEATURE_NO_CYGWIN_SYMLINK)
	...
	TRY {
		...
		node->fn_clusterv    = (FatClusterIndex *)kmalloc(sizeof(FatClusterIndex), FS_GFP);
		node->fn_clustera    = 1;
		node->fn_clusterc    = 1;
		node->fn_clusterv[0] = super->ft_cluster_eof_marker;
		TRY {
			...
			/* Try to free the already-written (and allocated) new-directory pattern. */
			Fat_DeleteClusterChain((FatSuperblock *)target_directory->i_super,
			                       link_node->i_fsdata->fn_clusterv[0]);
			RETHROW();
		}
	} EXCEPT {
		kfree(link_node->i_fsdata->fn_clusterv);
...
Fat12_SetFatIndirection(FatSuperblock *__restrict self,
	...
	assertf(index < self->ft_fat_length,
	        "Out-of-bounds FAT index: %" PRIu32 " >= %" PRIu32 "",
	        index, self->ft_fat_length);
	pval = ((u16 *)((byte_t const *)self->ft_fat_table + (index + (index / 2))));
	if (index & 1)
...
Fat16_GetFatIndirection(FatSuperblock const *__restrict self,
	...
	assertf(index < self->ft_fat_length,
	        "Out-of-bounds FAT index: %" PRIu32 " >= %" PRIu32 "",
	        index, self->ft_fat_length);
	return ((u16 *)self->ft_fat_table)[index];
}
...
Fat32_GetFatIndirection(FatSuperblock const *__restrict self,
	...
	assertf(index < self->ft_fat_length,
	        "Out-of-bounds FAT index: %" PRIu32 " >= %" PRIu32 "",
	        index, self->ft_fat_length);
	return ((u32 *)self->ft_fat_table)[index];
}
...
Fat_GetFatIndirection(FatSuperblock *__restrict self,
	...
	table_sector = (*self->ft_fat_sector)(self, index);
	assertf(table_sector < self->ft_sec4fat,
	        "Out-of-bounds FAT sector index: %" PRIu32 " >= %" PRIu32 "",
	        table_sector, self->ft_sec4fat);
	if (!FAT_META_GTLOAD(self, table_sector)) {
		/* Must load missing table sectors. */
		SCOPED_WRITELOCK(&self->ft_fat_lock);
		/* Lazily load the fat table sector that hasn't been loaded yet. */
		block_device_read(self->s_device,
		                  (byte_t *)self->ft_fat_table + (table_sector << self->ft_sectorshift),
		                  self->ft_sectorsize,
		                  FAT_SECTORADDR(self, self->ft_fat_start + table_sector));
		FAT_META_STLOAD(self, table_sector);
	}
	/* Now just read the FAT entry. */
	return (*self->ft_fat_get)(self, index);
}
...
Fat_SetFatIndirection(FatSuperblock *__restrict self,
                      ...
                      FatClusterIndex indirection_target) {
	FatSectorIndex table_sector;
	assert(sync_writing(&self->ft_fat_lock));
	table_sector = (*self->ft_fat_sector)(self, index);
	assertf(table_sector < self->ft_sec4fat,
	        "Out-of-bounds FAT sector index: %" PRIu32 " >= %" PRIu32 "",
	        table_sector, self->ft_sec4fat);
...
Fat_FindFreeCluster(FatSuperblock *__restrict self) {
	FatClusterIndex result;
	assert(sync_reading(&self->ft_fat_lock));
...
Fat_OpenSuperblock(FatSuperblock *__restrict self, UNCHECKED USER char *args)
		...
		THROW(E_FSERROR_WRONG_FILE_SYSTEM);
	{
		...
		self->ft_sectorsize = sector_size;
		_mfile_init_blockshift(self, CTZ(sector_size));
	}
	/* Extract some common information. */
	self->ft_fat_start   = (FatSectorIndex)LETOH16(disk_header.bpb.bpb_reserved_sectors);
	self->ft_sec4clus    = (size_t)disk_header.bpb.bpb_sectors_per_cluster;
	self->ft_fat_count   = (u32)disk_header.bpb.bpb_fatc;
	self->ft_clustersize = (size_t)self->ft_sec4clus << self->ft_sectorshift;

	/* Figure out what kind of FAT filesystem this is. */
	if (!disk_header.bpb.bpb_sectors_per_fat || !disk_header.bpb.bpb_maxrootsize) {
		self->ft_dat_start = self->ft_fat_start + (LETOH32(disk_header.fat32.f32_sectors_per_fat) *
		                                         disk_header.bpb.bpb_fatc);
		self->ft_type      = FAT32;
	} else {
		u32 fat_size, root_sectors;
		u32 data_sectors, total_clusters;
		root_sectors = (LETOH16(disk_header.bpb.bpb_maxrootsize) * sizeof(struct fat_dirent) +
		                (self->ft_sectorsize - 1)) >>
		               self->ft_sectorshift;
		fat_size          = (disk_header.bpb.bpb_fatc * LETOH16(disk_header.bpb.bpb_sectors_per_fat));
		self->ft_dat_start = LETOH16(disk_header.bpb.bpb_reserved_sectors);
		self->ft_dat_start += fat_size;
		self->ft_dat_start += root_sectors;
	...
	if (self->ft_type == FAT32) {
		if (disk_header.fat32.f32_signature != 0x28 &&
		    disk_header.fat32.f32_signature != 0x29)
			THROW(E_FSERROR_WRONG_FILE_SYSTEM);
		self->ft_volid = LETOH32(disk_header.fat32.f32_volid);
		memcpy(self->ft_label, disk_header.fat32.f32_label, sizeof(disk_header.fat32.f32_label));
		memcpy(self->ft_sysname, disk_header.fat32.f32_sysname, sizeof(disk_header.fat32.f32_sysname));
		self->ft_sec4fat            = LETOH32(disk_header.fat32.f32_sectors_per_fat);
		self->ft_cluster_eof        = (self->ft_sec4fat << self->ft_sectorshift) / 4;
		self->ft_cluster_eof_marker = 0xffffffff;
		/* Must lookup the cluster of the root directory. */
		self->ft_root.fn_clusterc    = 1;
		self->ft_root.fn_clustera    = 2;
		self->ft_root.fn_clusterv    = (FatClusterIndex *)kmalloc(2 * sizeof(FatClusterIndex), FS_GFP);
		self->ft_root.fn_clusterv[0] = LETOH32(disk_header.fat32.f32_root_cluster);
		self->ft_fat_get            = &Fat32_GetFatIndirection;
		self->ft_fat_set            = &Fat32_SetFatIndirection;
		self->ft_fat_sector         = &Fat32_GetTableSector;
	} else {
		if (self->ft_type == FAT12) {
			self->ft_cluster_eof_marker = 0xfff;
			self->ft_fat_get            = &Fat12_GetFatIndirection;
			self->ft_fat_set            = &Fat12_SetFatIndirection;
			self->ft_fat_sector         = &Fat12_GetTableSector;
		} else {
			self->ft_cluster_eof_marker = 0xffff;
			self->ft_fat_get            = &Fat16_GetFatIndirection;
			self->ft_fat_set            = &Fat16_SetFatIndirection;
			self->ft_fat_sector         = &Fat16_GetTableSector;
		}
		if (disk_header.fat16.f16_signature != 0x28 &&
		    disk_header.fat16.f16_signature != 0x29)
			THROW(E_FSERROR_WRONG_FILE_SYSTEM);
		self->ft_volid = LETOH32(disk_header.fat16.f16_volid);
		memcpy(self->ft_label, disk_header.fat16.f16_label, sizeof(disk_header.fat16.f16_label));
		memcpy(self->ft_sysname, disk_header.fat16.f16_sysname, sizeof(disk_header.fat16.f16_sysname));
		self->ft_root.fn16_root.r16_rootpos = (pos_t)LETOH16(disk_header.bpb.bpb_reserved_sectors);
		self->ft_root.fn16_root.r16_rootpos += (disk_header.bpb.bpb_fatc * LETOH16(disk_header.bpb.bpb_sectors_per_fat));
		self->ft_root.fn16_root.r16_rootpos <<= self->ft_sectorshift;
		self->ft_sec4fat                   = LETOH16(disk_header.bpb.bpb_sectors_per_fat);
		self->ft_root.fn16_root.r16_rootsiz = (u32)LETOH16(disk_header.bpb.bpb_maxrootsize);
		self->ft_root.fn16_root.r16_rootsiz *= sizeof(struct fat_dirent);
		self->ft_cluster_eof = (self->ft_sec4fat << self->ft_sectorshift) / 2;
		...
		if (self->ft_cluster_eof > self->ft_cluster_eof_marker)
			self->ft_cluster_eof = self->ft_cluster_eof_marker;
	}
	if (self->ft_cluster_eof <= 0xffff)
		self->ft_features |= FAT_FEATURE_ARB; /* 16-bit clusters --> high 16 bits are the ARB mask. */
	if (self->ft_cluster_eof_marker < self->ft_cluster_eof)
		self->ft_cluster_eof_marker = self->ft_cluster_eof;
	/* XXX: Detect `FAT_FEATURE_UGID' */

	self->ft_fat_table = NULL;
	self->ft_fat_meta  = NULL;
	COMPILER_WRITE_BARRIER();
	self->i_fsdata = &self->ft_root;

	memcpy(&self->ft_oem, disk_header.bpb.bpb_oem, 8, sizeof(char));
	self->ft_fat_size = (size_t)self->ft_sec4fat << self->ft_sectorshift;
	trimspecstring(self->ft_oem, 8);
	trimspecstring(self->ft_label, 11);
	trimspecstring(self->ft_sysname, 8);

	self->ft_uid  = 0;
	self->ft_gid  = 0;
	self->ft_mode = 0755; /* Read/Execute permissions for everyone. */
	...
	self->ft_free_pos = 0;
	TRY {
		self->ft_fat_table = kmalloc(self->ft_fat_size, FS_GFP);
		self->ft_fat_meta = (uintptr_t *)kmalloc(CEILDIV(self->ft_sec4fat, BITS_PER_POINTER / FAT_METABITS) *
		                                        sizeof(uintptr_t),
		                                        FS_GFP | GFP_CALLOC);
	} EXCEPT {
		/*kfree(self->ft_fat_meta);*/
		kfree(self->ft_fat_table);
		if (self->ft_type == FAT32)
			kfree(self->ft_root.fn_clusterv);
		RETHROW();
	}
	rwlock_cinit(&self->ft_fat_lock);
	printk(KERN_INFO "[fat] Load FAT%u-filesystem on "
	       ...
	       self->ft_type == FAT12 ? 12 : self->ft_type == FAT16 ? 16 : 32,
	       MAJOR(blkdev_devno(self->s_device)),
	       MINOR(blkdev_devno(self->s_device)),
	       self->s_device->bd_name,
	       self->ft_oem, self->ft_label, self->ft_sysname);
}
...
Fat_OpenINode(FatSuperblock *__restrict UNUSED(self),
	...
	node->i_fsdata = (FatNodeData *)kmalloc(sizeof(FatNodeData), FS_GFP | GFP_CALLOC);
}
...
Fat_WriteFatIndirectionTableSegment(FatSuperblock *__restrict self,
	...
	assertf(fat_sector_index + num_sectors <= self->ft_sec4fat,
	        "Out-of-bounds FAT sector index: %" PRIu32 "+%" PRIu32 "(%" PRIu32 ") > %" PRIu32 "",
	        fat_sector_index, num_sectors, fat_sector_index + num_sectors, self->ft_sec4fat);
	sector_buffer = (byte_t *)self->ft_fat_table + ((size_t)fat_sector_index << self->ft_sectorshift);
	sector_bytes  = (size_t)num_sectors << self->ft_sectorshift;
	sector_start  = self->ft_fat_start + fat_sector_index;
	printk(KERN_INFO "[fat] Saving modified meta-sectors "
	                 "%" PRIu32 "..%" PRIu32 " of 0..%" PRIu32 " (%" PRIu32 "..%" PRIu32 ")\n",
	       fat_sector_index, fat_sector_index + num_sectors - 1, self->ft_sec4fat - 1,
	       sector_start, sector_start + num_sectors - 1);
	/* Write to all redundant FAT copies. */
	for (i = 0; i < self->ft_fat_count; ++i) {
...
Fat_StatSuperblock(FatSuperblock *__restrict self,
	...
	result->f_bsize  = self->ft_sectorsize;
	result->f_blocks = self->ft_fat_length;
	result->f_bfree  = 0; /* TODO */
	result->f_bavail = 0; /* TODO */
	memcpy(&result->f_fsid, self->ft_label, sizeof(result->f_fsid));
	result->f_namelen = LFN_SEQNUM_MAXCOUNT * LFN_NAME;
	result->f_frsize  = self->ft_sectorsize; /* ??? */
}
...
Fat_SynchronizeSuperblock(FatSuperblock *__restrict self)
	...
	if (self->ft_flags & FAT_FCHANGED) {
		...
		for (;;) {
			...
			while (changed_start < self->ft_sec4fat && !FAT_META_GTCHNG(self, changed_start))
				++changed_start;
			changed_end = changed_start;
			while (changed_end < self->ft_sec4fat && FAT_META_GTCHNG(self, changed_end))
				++changed_end;
			if (changed_end == changed_start) {
				assert(changed_start == self->ft_sec4fat);
				assert(changed_end == self->ft_sec4fat);
				break;
			}
...
#endif /* !GUARD_MODFAT_OLDFATFAT_C */
