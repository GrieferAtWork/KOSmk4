...
INTERN size_t KCALL
handle_file_aread(struct file *__restrict self,
                  USER CHECKED void *dst,
                  size_t num_bytes,
                  iomode_t mode,
...
INTERN size_t KCALL
handle_file_awrite(struct file *__restrict self,
                   USER CHECKED void const *src,
                   size_t num_bytes,
                   iomode_t mode,
                   struct aio_multihandle *__restrict aio) {
	...
	/* TODO: This implementation of `IO_APPEND' isn't thread-safe! */
	dst_pos = mode & IO_APPEND ? ATOMIC_READ(self->f_node->i_filesize) : old_pos;
	inode_awrite(self->f_node, src, num_bytes, dst_pos, aio);
	if (unlikely(!ATOMIC_CMPXCH(self->f_offset, old_pos, dst_pos + num_bytes)) && num_bytes) {
		/* XXX: What now? */
	}
	return num_bytes;
}
...
INTERN size_t KCALL
handle_file_apread(struct file *__restrict self,
                   USER CHECKED void *dst,
                   size_t num_bytes,
                   pos_t addr, iomode_t mode,
                   struct aio_multihandle *__restrict aio) {
...
INTERN size_t KCALL
handle_file_apwrite(struct file *__restrict self,
                    USER CHECKED void const *src,
                    size_t num_bytes,
                    pos_t addr, iomode_t mode,
                    struct aio_multihandle *__restrict aio) {
	(void)mode;
	inode_awrite(self->f_node, src, num_bytes, addr, aio);
	return num_bytes;
}
...
INTERN size_t KCALL
handle_file_awritev(struct file *__restrict self,
                    struct aio_buffer *__restrict src,
                    size_t num_bytes,
                    iomode_t mode,
                    struct aio_multihandle *__restrict aio) {
	...
	/* TODO: This implementation of `IO_APPEND' isn't thread-safe! */
	dst_pos = mode & IO_APPEND ? ATOMIC_READ(self->f_node->i_filesize) : old_pos;
	inode_awritev(self->f_node, src, num_bytes, dst_pos, aio);
	if (unlikely(!ATOMIC_CMPXCH(self->f_offset, old_pos, dst_pos + num_bytes)) && num_bytes) {
		/* XXX: What now? */
	}
	return num_bytes;
...
INTERN size_t KCALL
handle_file_apreadv(struct file *__restrict self,
                    struct aio_buffer *__restrict dst,
                    size_t num_bytes,
                    pos_t addr, iomode_t mode,
                    struct aio_multihandle *__restrict aio) {
...
INTERN size_t KCALL
handle_file_apwritev(struct file *__restrict self,
                     struct aio_buffer *__restrict src,
                     size_t num_bytes,
                     pos_t addr, iomode_t mode,
                     struct aio_multihandle *__restrict aio) {
	(void)mode;
	inode_awritev(self->f_node, src, num_bytes, addr, aio);
	return num_bytes;
}
