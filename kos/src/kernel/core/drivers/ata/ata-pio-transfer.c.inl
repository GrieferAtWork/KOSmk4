...
#define PAGEDIR_MAPPING_FLAGS   PAGEDIR_MAP_FREAD
	...
	TRY {
		for (;;) {
			...
			page_words = (pagedir_pagesize() - (buf & (pagedir_pagesize() - 1))) / 2;
			if (page_words > count)
				page_words = count;
			if (is_first) {
			...
			{
				size_t i;
				u16 *src = (u16 *)(VM_PAGE2ADDR(tramp) + (ptrdiff_t)(buf & (pagedir_pagesize() - 1)));
				for (i = 0; i < page_words; ++i)
					outw(port, *src++);
			}
#else
			outsw(port, (void *)(VM_PAGE2ADDR(tramp) + (ptrdiff_t)(buf & (pagedir_pagesize() - 1))), page_words);
#endif
			if (page_words >= count)
				break;
			count -= page_words;
