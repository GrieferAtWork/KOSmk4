 ...
 */
#ifndef GUARD_KERNEL_CORE_ARCH_I386_SCHED_OLDPOSIX_SIGNAL_C
#define GUARD_KERNEL_CORE_ARCH_I386_SCHED_OLDPOSIX_SIGNAL_C 1
...
#include "oldposix-signal-raise.c.inl"
...
PRIVATE struct icpustate *FCALL
sys_sigreturn32_rpc(void *UNUSED(arg),
                    struct icpustate *__restrict state,
                    unsigned int reason,
                    struct rpc_syscall_info const *sc_info) {
	if unlikely(reason != TASK_RPC_REASON_SYSCALL)
		return state;
	return sys_sigreturn32_impl(state,
...
#endif /* !GUARD_KERNEL_CORE_ARCH_I386_SCHED_OLDPOSIX_SIGNAL_C */
