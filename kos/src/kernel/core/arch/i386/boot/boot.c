...
NOTHROW(KCALL __i386_kernel_main)(struct icpustate *__restrict state) {
	...
	/* TODO: `merge_all_parts()' scales  horribly, and  there has  to be  a
	 *       better  fallback  system  than  "let's just merge everything".
	 *       Maybe it would be wise to add another pointer-field to `mpart'
	 *       for  the sole purpose  of chaining parts  for a situation like
	 *       this or the fallback of the planned `mpart_unload()'.
	 *
	 * That way, there could be a secondary global list of parts for which
	 * some sort of async operation is pending, which are singly-linked by
	 * this new link-field. */
