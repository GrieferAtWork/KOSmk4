 ...
 */
#ifndef GUARD_KERNEL_CORE_ARCH_I386_FPU_C
#define GUARD_KERNEL_CORE_ARCH_I386_FPU_C 1
...
x86_cirq_07(struct icpustate *__restrict state) {
	...
	if (new_task == old_task) {
		...
		printk(KERN_INFO "Switch FPU context from task %p[pid=%u] to %p[pid=%u] [pc=%p]\n",
		       ...
		       new_task, task_getroottid_of_s(new_task), ICPUSTATE_PC(*state));
		mystate = PERTASK_GET(_this_fpustate);
		if (!mystate) {
...
#endif /* !GUARD_KERNEL_CORE_ARCH_I386_FPU_C */
