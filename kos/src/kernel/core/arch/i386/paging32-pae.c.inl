 ...
 */
#ifndef GUARD_KERNEL_CORE_ARCH_I386_PAGING32_PAE_C_INL
#define GUARD_KERNEL_CORE_ARCH_I386_PAGING32_PAE_C_INL 1
...
NOTHROW(FCALL pae_pagedir_fini)(VIRT struct pae_pdir *__restrict self,
	...
	for (vec3 = 0; vec3 < 3; ++vec3) {
		...
		for (vec2 = 0; vec2 < 512; ++vec2) {
			...
			if unlikely(PAE_PDIR_E2_ISVEC1(e2.p_word))
				page_free(VM_ADDR2PAGE((vm_phys_t)(e2.p_word & PAE_PAGE_FVECTOR)), 1);
		}
	}
	...
	for (vec3 = 0; vec3 < 4; ++vec3) {
		...
		page_free(VM_ADDR2PAGE((vm_phys_t)(self->p_e3[vec3].p_word & PAE_PAGE_FVECTOR)), 1);
	}
}
...
again:
	...
	if (!e2.p_vec1.v_present) {
		...
		backup = pae_pagedir_push_mapone(tramp, new_e1_vector,
		                                 PAGEDIR_MAP_FWRITE);
		pagedir_syncone(tramp);
		e1_p = (union pae_pdir_e1 *)VM_PAGE2ADDR(tramp);
		...
		COMPILER_WRITE_BARRIER();
		pae_pagedir_pop_mapone(tramp, backup);
		/* Map the new vector. */
		new_e2_word = VM_PAGE2ADDR((u64)new_e1_vector) |
...
NOTHROW(FCALL pae_pagedir_unprepare_impl_flatten)(unsigned int vec3,
	...
	old_version = ATOMIC_READ64(x86_pagedir_prepare_version);
...
LOCAL NOBLOCK void
NOTHROW(FCALL pae_pagedir_andl_e1_word)(unsigned int vec3,
                                        unsigned int vec2,
                                        unsigned int vec1,
...
NOTHROW(FCALL pae_pagedir_maphintone)(VIRT vm_vpage_t virt_page,
	...
	assert(IS_ALIGNED((uintptr_t)hint, PAE_PAGEDIR_MAPHINT_ALIGNMENT));
	vec3 = PAE_PDIR_VEC3INDEX_VPAGE(virt_page);
	vec2 = PAE_PDIR_VEC2INDEX_VPAGE(virt_page);
	vec1 = PAE_PDIR_VEC1INDEX_VPAGE(virt_page);
...
NOTHROW(FCALL pae_pagedir_gethint)(VIRT vm_vpage_t virt_page) {
	...
	word = ATOMIC_READ(*(uintptr_t *)&PAE_PDIR_E1_IDENTITY[vec3][vec2][vec1].p_word);
	if unlikely(!PAE_PDIR_E1_ISHINT(word))
		return NULL;
...
INTERN NOBLOCK void
NOTHROW(FCALL pae_pagedir_pop_mapone)(VIRT vm_vpage_t virt_page, uintptr_t backup) {
	u64 old_word;
	unsigned int vec3, vec2, vec1;
	vec3 = PAE_PDIR_VEC3INDEX_VPAGE(virt_page);
	vec2 = PAE_PDIR_VEC2INDEX_VPAGE(virt_page);
...
NOTHROW(FCALL pae_pagedir_unwriteone)(VIRT vm_vpage_t virt_page) {
	...
	vec2 = PAE_PDIR_VEC2INDEX_VPAGE(virt_page);
	vec1 = PAE_PDIR_VEC1INDEX_VPAGE(virt_page);
	pae_pagedir_andl_e1_word(vec3, vec2, vec1, ~PAE_PAGE_FWRITE);
}
...
#endif /* !GUARD_KERNEL_CORE_ARCH_I386_PAGING32_PAE_C_INL */
