...
again:
	...
	TRY {
		 ...
		 *       sc_info->rsi_regs[0] = fd
		 *       sc_info->rsi_regs[1] = offset & 0xffffffff
		 *       sc_info->rsi_regs[2] = (offset >> 32) & 0xffffffff
		 *       sc_info->rsi_regs[3] = whence
		 *       As such, we'd need to invoke its C-level function as follows:
		 *       result = sys32_lseek64(sc_info->rsi_regs[0],
		 *                              sc_info->rsi_regs[1] | sc_info->rsi_regs[2] << 32,
		 *                              sc_info->rsi_regs[3]);
