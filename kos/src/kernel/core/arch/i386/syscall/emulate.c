...
again:
	...
	TRY {
#ifdef __x86_64__
		/* TODO: Deal with double-wide argument registers!
		 *       When emulating a 32-bit system call such as lseek64(), sc_info will look like this:
		 *       sc_info->rsi_args[0] = fd
		 *       sc_info->rsi_args[1] = offset & 0xffffffff
		 *       sc_info->rsi_args[2] = (offset >> 32) & 0xffffffff
		 *       sc_info->rsi_args[3] = whence
		 *       As such, we'd need to invoke its C-level function as follows:
		 *       result = sys32_lseek64(sc_info->rsi_args[0],
		 *                              sc_info->rsi_args[1] | sc_info->rsi_args[2] << 32,
		 *                              sc_info->rsi_args[3]);
		 *       This could be implemented by generating another set of system call wrapper functions
		 *       similar to those already generated by `x86_64_printKernelSyscallWrapper_asm32_int80()'
		 *       and `x86_64_printKernelSyscallWrapper_asm32_sysenter()', only these would implement
		 *       SYSVABI-compatible wrapper functions (such as `sysrun32_setuid()') when doing so is
		 *       necessary (i.e. whenever the system call has no double-wide register, and doesn't takes
		 *       an argument that is smaller than 32-bit, `sysrun32_NAME()' should be defined alongside the
		 *       definition of `sys32_NAME()', and be an alias for the later; s.a. `DEFINE_SYSCALLn' macros)
		 *       These wrapper functions would then be given another routing table similar to `x86_sysrouteN_c32',
		 *       and we'll then be calling them via that table from right here. (Also note that with this model,
		 *       the `x86_sysrouteN_c32' router could be removed completely, as there aren't any cases where
		 *       it would still be required).
		 *       -> This would then be able to solve the issue of double-wide registers, as well as
		 *          registers that are smaller than 32-bits
		 *
		 * TODO: On a related note, rename `rpc_syscall_info::rsi_args' to `rpc_syscall_info::rsi_regs',
		 *       since that's want it actually is, as opposed to being the raw arguments...
		 */
#endif /* __x86_64__ */
