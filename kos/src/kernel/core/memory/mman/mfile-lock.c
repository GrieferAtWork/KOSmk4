...
#define mfile_unlock_and_decref_parts_s(self)                        \
	(((self)->mf_parts && (self)->mf_parts != MFILE_PARTS_ANONYMOUS) \
	 ? mpart_unlock_and_decref_whole_tree((self)->mf_parts)          \
	 : (void)0)
#define mfile_unlock_and_decref_parts_except_s(self, part)               \
	(((self)->mf_parts && (self)->mf_parts != MFILE_PARTS_ANONYMOUS)     \
	 ? mpart_unlock_and_decref_whole_tree_except((self)->mf_parts, part) \
	 : (void)0)
...
/* Release locks to all msalign files of `self' before (but not including) `stop' */
PRIVATE NOBLOCK NONNULL((1, 2)) void
NOTHROW(FCALL mfile_unlock_and_decref_parts_msalign_until)(struct mfile *__restrict self,
                                                           struct misaligned_mfile *__restrict stop) {
	struct misaligned_mfile *first, *iter;
	first = LIST_FIRST(&self->mf_msalign);
	if (first == stop)
		return;
	iter = LIST_PREV_UNSAFE(stop, mam_link);
	for (;;) {
		struct misaligned_mfile *prev;
		mfile_unlock_and_decref_parts_s(iter);
		prev = LIST_PREV_UNSAFE(iter, mam_link);
...
NOTHROW(FCALL mfile_unlock_and_decref_parts_with_msalign)(struct mfile *__restrict self) {
	if unlikely(!LIST_EMPTY(&self->mf_msalign)) {
		struct misaligned_mfile *iter;
		iter = LIST_FIRST(&self->mf_msalign);
		do {
			mfile_unlock_and_decref_parts_s(iter);
			mfile_lock_endwrite(iter);
			decref_unlikely(iter);
		} while ((iter = LIST_NEXT(iter, mam_link)) != NULL);
	}
...
NOTHROW(FCALL mfile_unlock_and_decref_parts_with_msalign_except)(struct mfile *__restrict self,
	...
	if unlikely(!LIST_EMPTY(&self->mf_msalign)) {
		...
		iter = LIST_FIRST(&self->mf_msalign);
		do {
			mfile_unlock_and_decref_parts_except_s(iter, part);
			mfile_lock_endwrite(iter);
			decref_unlikely(iter);
		} while ((iter = LIST_NEXT(iter, mam_link)) != NULL);
	}
...
PUBLIC NOBLOCK WUNUSED NONNULL((1)) REF void *
NOTHROW(FCALL mfile_tryincref_and_lock_parts_with_msalign)(struct mfile *__restrict self) {
	REF struct mpart *bpart;
	bpart = mfile_tryincref_and_lock_parts(self);
	if (bpart != NULL)
		return mfile_tryincref_and_lock_parts_with_msalign__forpart(bpart);
	if unlikely(!LIST_EMPTY(&self->mf_msalign)) {
		struct misaligned_mfile *iter;
		iter = LIST_FIRST(&self->mf_msalign);
		do {
			/* Try to acquire a reference to this sub-file. */
			if unlikely(!tryincref(iter)) {
				struct misaligned_mfile *next;
				/* Dead sub-file -> remove */
				next = LIST_NEXT(iter, mam_link);
				LIST_UNBIND(iter, mam_link);
				iter = next;
				goto continue_with_iter;
			}

			/* Try to acquire a lock to this sub-file. */
			if (!mfile_lock_trywrite(iter)) {
				mfile_unlock_and_decref_parts_msalign_until(self, iter);
				mfile_unlock_and_decref_parts(self);
				return mfile_tryincref_and_lock_parts_with_msalign__forfile(iter);
			}

			/* If the sub-file isn't anonymous (very likely), then also incref+lock all of *its* parts. */
			if likely(iter->mf_parts != MFILE_PARTS_ANONYMOUS) {
				bpart = mfile_tryincref_and_lock_parts(iter);
				if (bpart != NULL) {
					mfile_lock_endwrite(iter);
					decref_unlikely(iter);
					mfile_unlock_and_decref_parts_msalign_until(self, iter);
