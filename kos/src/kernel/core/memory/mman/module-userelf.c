...
INTERN_CONST struct module_ops const uem_ops = {
	...
	.mo_locksection       = (WUNUSED_T NONNULL_T((1)) REF struct module_section *NOTHROW_T(FCALL *)(struct module *__restrict, NCX char const *))&uem_locksection,
...
INTERN BLOCKING WUNUSED NONNULL((1)) NCX byte_t *FCALL
...
again:
	...
	result->ums_useraddr = (NCX byte_t *)-1;
	...
	if (result->ms_flags & SHF_ALLOC) {
		...
		result->ums_useraddr = (NCX byte_t *)(self->md_loadaddr +
...
uem_create_system_rtld(struct mman *__restrict self,
                       ...
                       struct fdirent *__restrict rtld_name,
                       NCX byte_t *node_addr, pos_t node_fpos) {
	REF struct userelf_module *result;
