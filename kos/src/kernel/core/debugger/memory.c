...
NOTHROW(KCALL dbg_readmemory)(NCX UNCHECKED void const *addr,
                              void *__restrict buf,
                              size_t num_bytes) {
	size_t error;
	if (!num_bytes)
...
NOTHROW(KCALL dbg_writememory)(NCX UNCHECKED void *addr,
...
PUBLIC size_t
NOTHROW(KCALL dbg_setmemory)(NCX UNCHECKED void *addr, byte_t byte,
                             size_t num_bytes, bool force) {
	byte_t *buf;
