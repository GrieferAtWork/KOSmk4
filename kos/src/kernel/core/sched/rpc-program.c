...
struct rpc_membank {
	NCX byte_t                     *rmb_addrlo;  /* [const] Lowest bank address. (1-byte granularity) */
	NCX byte_t                     *rmb_addrhi;  /* [const] Greatest bank address. */
...
rpc_membank_create(NCX byte_t *addr, size_t num_bytes)
		THROWS(E_BADALLOC) {
...
struct rpc_futex {
	NCX void *rf_addr; /* [const] Futex address. */
...
LOCAL void FCALL verify_writable_byte(NCX byte_t *addr) {
	byte_t buf;
...
struct rpc_vm {
	...
	NCX byte_t const              *rv_pc;                        /* [?..1] User-space address of next PC byte. */
...
#define rpc_vm_getsp(self) ((NCX UNCHECKED byte_t *)ucpustate_getsp(&(self)->rv_cpu))
#else /* ucpustate_getsp */
PRIVATE WUNUSED NONNULL((1)) NCX UNCHECKED byte_t *FCALL
...
PRIVATE NONNULL((1)) void FCALL
rpc_vm_setsp(struct rpc_vm *__restrict self, NCX UNCHECKED byte_t *value) {
#ifdef NDEBUG
...
follow_jmp:
	...
	CASE(RPC_OP_futex_wake) {
		...
		ftx = rpc_mem_getfutex(&self->rv_mem, (NCX UNCHECKED void *)addr);
		if (ftx) {
...
task_userrpc_runprogram(rpc_cpustate_t *__restrict state,
	...
	vm.rv_pc        = (NCX byte_t *)rpc->pr_user.pur_prog;
	vm.rv_pcbuf_siz = 0;
