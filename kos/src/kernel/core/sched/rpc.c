...
STATIC_ASSERT(BLOCKING_CLEANUP_SERVICE_NONE == TASK_SERVE_NX_NOOP);
STATIC_ASSERT(BLOCKING_CLEANUP_SERVICE_XRPC == TASK_SERVE_NX_XPENDING);
...
#define SET_STATUS(code)                                 \
		...
		TRY {
			if (reason == TASK_RPC_REASON_SHUTDOWN) {
				...
				else if (SYSCALL_RESTART_MODE(sc_info->rsi_sysno) != SYSCALL_RESTART_MODE_DONT)
