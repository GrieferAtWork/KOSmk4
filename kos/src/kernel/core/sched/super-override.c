 ...
 * >> PERTASK_SET(this_task.t_state, state);
 * >> sched_super_override_ipi();
 * >> sched_override_end();
 * >> state = PERTASK_GET(this_task.t_state);
...
INTERN NOPREEMPT NOBLOCK ATTR_COLDTEXT void
...
PRIVATE NOBLOCK NOCONNECT bool FCALL
...
PUBLIC NOBLOCK NOCONNECT void FCALL
