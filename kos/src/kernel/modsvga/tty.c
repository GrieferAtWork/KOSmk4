...
svga_ansitty_v_ioctl(struct mfile *__restrict self, syscall_ulong_t cmd,
                     USER UNCHECKED void *arg, iomode_t mode) THROWS(...) {
	struct svgatty *me = (struct svgatty *)mfile_asansitty(self);
	/* TODO: the promised ioctls changing video modes. */
	(void)me;
...
PRIVATE struct mfile_stream_ops const svga_ansitty_v_stream_ops = {
	.mso_write = &ansittydev_v_write,
	.mso_ioctl = &svga_ansitty_v_ioctl,
...
INTERN_CONST struct ansittydev_ops const svgatty_ops = {{{{{
	.no_file = {
		...
		.mo_stream  = &svga_ansitty_v_stream_ops,
	},
