...
got_identify_signal:
		...
		*(char *)mempcpy(drive->bd_rootinfo.br_ata_model, specs.model, 40) = '\0';
		...
		if (is_default_ide) {
			drive->dn_devno  = MKDEV(is_primary_bus ? 3 : 22, drive_id == ATA_DRIVE_MASTER ? 0 : 64);
			drive->dv_dirent = fdevfsdirent_newf("hd%c", (is_primary_bus ? 'a' : 'c') + (drive_id == ATA_DRIVE_MASTER ? 0 : 1));
		} else {
			drive->dn_devno  = MKDEV(DEV_MAJOR_AUTO, 0);
			drive->dv_dirent = fdevfsdirent_newf("hdX"
			                                     "%" PRIxN(__SIZEOF_PORT_T__) "."
			                                     "%" PRIxN(__SIZEOF_PORT_T__) "."
			                                     "%" PRIxN(__SIZEOF_PORT_T__) "."
			                                     "%c",
