...
#define DBG_REGLEVEL_ORIG 2 /* The register state loaded by `dbg_current' when it resumes.
                             * When `dbg_current == THIS_TASK', same as `DBG_REGLEVEL_TRAP' */
...
FUNDEF bool NOTHROW(LIBUNWIND_CC dbg_getreg)(/*uintptr_t level*/ void const *arg, uintptr_half_t regno, void *__restrict buf);
FUNDEF bool NOTHROW(LIBUNWIND_CC dbg_setreg)(/*uintptr_t level*/ void *arg, uintptr_half_t regno, void const *__restrict buf);

LOCAL uintptr_t
NOTHROW(KCALL dbg_getregp)(unsigned int level, uintptr_half_t regno) {
	uintptr_t result;
#ifdef NDEBUG
	dbg_getreg((void *)(uintptr_t)level, regno, &result);
#else /* NDEBUG */
	bool ok;
	__hybrid_assert(CFI_REGISTER_SIZE(regno) == sizeof(uintptr_t));
	ok = dbg_getreg((void *)(uintptr_t)level, regno, &result);
	__hybrid_assert(ok);
#endif /* !NDEBUG */
	return result;
}
...
LOCAL void
NOTHROW(KCALL dbg_setregp)(unsigned int level, uintptr_half_t regno, uintptr_t value) {
#ifdef NDEBUG
	dbg_setreg((void *)(uintptr_t)level, regno, &value);
#else /* NDEBUG */
	bool ok;
	__hybrid_assert(CFI_REGISTER_SIZE(regno) == sizeof(uintptr_t));
	ok = dbg_setreg((void *)(uintptr_t)level, regno, &value);
	__hybrid_assert(ok);
#endif /* !NDEBUG */
}
