...
FORCELOCAL u32 NOTHROW(FCALL pci_read)(pci_addr_t base, pci_reg_t reg) { assert(!(base&PCI_ADDR_REGMASK)); return pci_readaddr(base|reg); }
FORCELOCAL void NOTHROW(FCALL pci_write)(pci_addr_t base, pci_reg_t reg, u32 value) { assert(!(base&PCI_ADDR_REGMASK)); pci_writeaddr(base|reg,value); }
#endif /* __CC__ */
