...
DATDEF ATTR_PERTASK REF struct ioperm_bitmap *_this_ioperm_bitmap;
#define THIS_IOPERM_BITMAP PERTASK_GET(_this_ioperm_bitmap)

/* [0..1][== NULL || == THIS_IOPERM_BITMAP]
 ...
 * was found to be non-NULL, and during the first access made to `x86_cpuiob' during
 * the life-time of thread, this variable will be filled with `THIS_IOPERM_BITMAP'.
 * It's main purpose is to keep track of the is-mapped state of the `x86_cpuiob'
 * memory area for the duration of some thread's quantum. */
DATDEF ATTR_PERCPU struct ioperm_bitmap *_current_ioperm_bitmap;
...
FUNDEF WUNUSED ATTR_RETNONNULL ATTR_MALLOC REF struct ioperm_bitmap *KCALL
ioperm_bitmap_alloc(void) THROWS(E_BADALLOC);
FUNDEF WUNUSED ATTR_RETNONNULL ATTR_MALLOC NOBLOCK_IF(flags & GFP_ATOMIC)
REF struct ioperm_bitmap *KCALL ioperm_bitmap_allocf(gfp_t flags) THROWS(E_BADALLOC);
FUNDEF WUNUSED ATTR_MALLOC NOBLOCK_IF(flags & GFP_ATOMIC)
...
FUNDEF WUNUSED ATTR_RETNONNULL ATTR_MALLOC NOBLOCK_IF(flags & GFP_ATOMIC) REF struct ioperm_bitmap *KCALL
ioperm_bitmap_copyf(struct ioperm_bitmap const *__restrict self, gfp_t flags) THROWS(E_BADALLOC);
FUNDEF WUNUSED ATTR_MALLOC NOBLOCK_IF(flags & GFP_ATOMIC) REF struct ioperm_bitmap *
