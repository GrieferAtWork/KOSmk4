 ...
 * @param: arg:       The same `arg' given to `mman_startdma[v]'
...
typedef __BOOL /*NOTHROW*/ (KCALL *mdma_range_callback_t)(void *arg, physaddr_t paddr, size_t num_bytes,
                                                          struct mdmalock *__restrict lock);
typedef void /*NOTHROW*/ (KCALL *mdma_reset_callback_t)(void *arg);

/* Descriptor for a lock held for the purposes of DMA */
struct mdmalock {
	REF struct mpart *mdl_part; /* [1..1] The data part to which a read-lock is being held. */
};
...
#define mman_dmalock_release(x) \
	(mpart_lock_release((x)->mdl_part), decref_unlikely((x)->mdl_part))
...
mman_startdma(struct mman *__restrict self,
              ...
              UNCHECKED void *vaddr, size_t num_bytes, __BOOL for_writing)
		THROWS(E_WOULDBLOCK, E_BADALLOC, ...);
FUNDEF NONNULL((1, 2, 3, 5, 7)) size_t KCALL
mman_startdmav(struct mman *__restrict self,
               mdma_range_callback_t prange, mdma_reset_callback_t preset,
               void *arg, struct mdmalock *__restrict lockvec, size_t lockcnt,
               struct aio_buffer const *__restrict vaddr_buf, __BOOL for_writing)
		THROWS(E_WOULDBLOCK, E_BADALLOC, ...);
