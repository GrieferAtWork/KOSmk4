 ...
 */
%[default:section(".text.crt.io.utility.kos")]

%[define_type_class(poll_mode_t = "TD")]
%[define_replacement(poll_mode_t = "unsigned int")]
...
poll_mode_t fd_polltest([[fdarg]] fd_t fd, poll_mode_t what);
...
[[wunused, decl_include("<bits/types.h>")]]
fd_t fd_cast([[fdarg]] fd_t fd, uint32_t type, iomode_t mode);
...
[[wunused, decl_include("<bits/types.h>")]]
fd_t fd_msalign([[fdarg]] fd_t fd, [[inout]] uint64_t *p_offset, iomode_t mode);
...
[[wunused, decl_include("<bits/types.h>")]]
fd_t fd_subregion([[fdarg]] fd_t fd, uint64_t base, uint64_t num_bytes, iomode_t mode);
...
int fd_delregion([[fdarg]] fd_t fd);
...
[[wunused, decl_include("<bits/types.h>")]]
ssize_t fd_tailread([[fdread]] fd_t fd, [[out(return <= bufsize)]] void *buf,
...
[[decl_include("<bits/types.h>")]]
uint64_t fd_trim([[fdarg]] fd_t fd, uint64_t base, uint64_t num_bytes, uint32_t mode);
