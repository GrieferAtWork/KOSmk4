...
INTDEF int NOTHROW_NCX(LIBCCALL libc_fegetexceptflag)(fexcept_t *flagp, int excepts);
/* Raise the supported exceptions represented by EXCEPTS */
INTDEF int NOTHROW(LIBCCALL libc_feraiseexcept)(int excepts);
/* Set complete status for exceptions indicated by EXCEPTS according
 * to the representation in the object pointed to by FLAGP */
INTDEF int NOTHROW_NCX(LIBCCALL libc_fesetexceptflag)(fexcept_t const *flagp, int excepts);
/* Determine which of subset of the exceptions
 * specified by EXCEPTS are currently set */
INTDEF int NOTHROW(LIBCCALL libc_fetestexcept)(int excepts);
/* Get current rounding direction */
INTDEF ATTR_PURE int NOTHROW(LIBCCALL libc_fegetround)(void);
...
INTDEF int NOTHROW_NCX(LIBCCALL libc_fegetenv)(fenv_t *envp);
/* Save the current environment in the object pointed to by ENVP, clear
 * exception flags and install a non-stop mode (if available) for all exceptions */
INTDEF int NOTHROW_NCX(LIBCCALL libc_feholdexcept)(fenv_t *envp);
/* Establish the floating-point environment represented by the object pointed to by ENVP */
INTDEF int NOTHROW_NCX(LIBCCALL libc_fesetenv)(fenv_t const *envp);
/* Save current exceptions in temporary storage, install environment
 * represented by object pointed to by ENVP and raise exceptions
 * according to saved exceptions */
INTDEF int NOTHROW_NCX(LIBCCALL libc_feupdateenv)(fenv_t const *envp);
...
INTDEF int NOTHROW(LIBCCALL libc_fegetexcept)(void);
