 ...
 * @return: -1:       Error (s.a. `errno') */
INTDEF NONNULL((2)) __STDC_INT_AS_SSIZE_T NOTHROW_RPC(LIBCCALL libc_epoll_wait)(fd_t epfd, struct epoll_event *events, __STDC_INT_AS_SIZE_T maxevents, int timeout);
/* >> epoll_pwait(2)
...
INTDEF NONNULL((3, 6)) int NOTHROW_NCX(LIBCCALL libc_epoll_rpc_exec)(fd_t epfd, fd_t fd, struct epoll_event *event, pid_t target_tid, unsigned int mode, prpc_exec_callback_t func);
#endif /* !__KERNEL__ */
