...
INTDEF NONNULL((3)) int NOTHROW_RPC(LIBDCALL libd_lfutexexpr)(void *base, size_t exprc, struct lfutexexpr const *exprv, struct timespec const *timeout, unsigned int timeout_flags);
/* >> lfutexlockexpr(2)
 ...
 * @return: -1:EINTR:     A blocking futex-wait operation was interrupted
 * @return: -1:ETIMEDOUT: A blocking futex-wait operation has timed out */
INTDEF NONNULL((1, 4)) int NOTHROW_RPC(LIBDCALL libd_lfutexlockexpr)(lfutex_t *ulockaddr, void *base, size_t exprc, struct lfutexexpr const *exprv, struct timespec const *timeout, unsigned int timeout_flags);
/* >> lfutexexpr(2)
 * The lfutexexpr()  system call  can be  used to  specify arbitrarily  complex
 * expressions  that must  atomically (in  relation to  other futex operations)
 * hold  true before the scheduler will suspend  the calling thread, as well as
 ...
 * @return: -1:EINTR:     A blocking futex-wait operation was interrupted
 * @return: -1:ETIMEDOUT: A blocking futex-wait operation has timed out */
INTDEF NONNULL((3)) int NOTHROW_RPC(LIBDCALL libd_lfutexexpr64)(void *base, size_t exprc, struct lfutexexpr const *exprv, struct timespec64 const *timeout, unsigned int timeout_flags);
/* >> lfutexlockexpr(2)
 * A  function that is similar to `lfutexexpr()', but  allows for the use of one central
 * locking futex that is used for waiting and may be distinct from any other given futex
 * object pointer.
 ...
 * @return: -1:EINTR:     A blocking futex-wait operation was interrupted
 * @return: -1:ETIMEDOUT: A blocking futex-wait operation has timed out */
INTDEF NONNULL((1, 4)) int NOTHROW_RPC(LIBDCALL libd_lfutexlockexpr64)(lfutex_t *ulockaddr, void *base, size_t exprc, struct lfutexexpr const *exprv, struct timespec64 const *timeout, unsigned int timeout_flags);
