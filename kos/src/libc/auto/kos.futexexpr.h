...
INTDEF ATTR_IN(3) ATTR_IN_OPT(4) NONNULL((1)) int (LIBCCALL libc_LFutexExprI)(lfutex_t *ulockaddr, void *base, struct lfutexexpr const *expr, struct timespec const *timeout, unsigned int timeout_flags) THROWS(...);
/* >> LFutexExprI(2), LFutexExprI64(2)
 * Same as `LFutexExpr()', but implicitly handle `E_INTERRUPT' exceptions by re-starting the system call.
 * @return: * :  The first  non-zero  return value  from  executing  all of  the  given  `expr'
 *               in order (s.a. the documentations of the individual `LFUTEX_WAIT_*'  functions
 ...
 * @throws: E_SEGFAULT:         A faulty pointer was given
 * @throws: E_INVALID_ARGUMENT: One of the given commands is invalid, or `expr[0].fe_condition == LFUTEX_EXPREND' */
INTDEF ATTR_IN(3) ATTR_IN_OPT(4) NONNULL((1)) int (LIBCCALL libc_LFutexExprI64)(lfutex_t *ulockaddr, void *base, struct lfutexexpr const *expr, struct timespec64 const *timeout, unsigned int timeout_flags) THROWS(...);
#endif /* !__KERNEL__ */
