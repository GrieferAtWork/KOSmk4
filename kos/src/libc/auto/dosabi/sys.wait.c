 ...
 */
#ifndef GUARD_LIBC_AUTO_DOSABI_SYS_WAIT_C
#define GUARD_LIBC_AUTO_DOSABI_SYS_WAIT_C 1
...
INTERN ATTR_SECTION(".text.crt.dos.sched.wait") pid_t
NOTHROW_RPC(LIBDCALL libd_wait)(__WAIT_STATUS stat_loc) {
	return libc_wait(stat_loc);
}
...
INTERN ATTR_SECTION(".text.crt.dos.sched.wait") pid_t
NOTHROW_RPC(LIBDCALL libd_waitpid)(pid_t pid,
                                   __WAIT_STATUS stat_loc,
                                   __STDC_INT_AS_UINT_T options) {
	return libc_waitpid(pid, stat_loc, options);
}
/* @param options: At least one of `WEXITED | WSTOPPED | WCONTINUED',
 *                 optionally or'd with `WNOHANG | WNOWAIT' */
INTERN ATTR_SECTION(".text.crt.dos.sched.wait") int
NOTHROW_RPC(LIBDCALL libd_waitid)(idtype_t idtype,
                                  id_t id,
                                  siginfo_t *infop,
                                  __STDC_INT_AS_UINT_T options) {
	return libc_waitid(idtype, id, infop, options);
}
/* Same as `waitpid(-1,STAT_LOC,OPTIONS)', though also fills in `USAGE' when non-NULL
 * @param options: Set of `WNOHANG | WUNTRACED | WCONTINUED' (as a KOS extension, `WNOWAIT' is also accepted) */
INTERN ATTR_SECTION(".text.crt.dos.sched.wait") pid_t
NOTHROW_RPC(LIBDCALL libd_wait3)(__WAIT_STATUS stat_loc,
                                 __STDC_INT_AS_UINT_T options,
                                 struct rusage *usage) {
	return libc_wait3(stat_loc, options, usage);
}
/* Same as `waitpid(-1,STAT_LOC,OPTIONS)', though also fills in `USAGE' when non-NULL
 * @param options: Set of `WNOHANG | WUNTRACED | WCONTINUED' (as a KOS extension, `WNOWAIT' is also accepted) */
INTERN ATTR_SECTION(".text.crt.dos.sched.wait") pid_t
NOTHROW_NCX(LIBDCALL libd_wait3_64)(__WAIT_STATUS stat_loc,
                                    __STDC_INT_AS_UINT_T options,
                                    struct rusage64 *usage) {
	return libc_wait3_64(stat_loc, options, usage);
}
/* Same as `waitpid(pid,STAT_LOC,OPTIONS)', though also fills in `USAGE' when non-NULL
 * @param options: Set of `WNOHANG|WUNTRACED|WCONTINUED' (as a KOS extension, `WNOWAIT' is also accepted) */
INTERN ATTR_SECTION(".text.crt.dos.sched.wait") pid_t
NOTHROW_RPC(LIBDCALL libd_wait4)(pid_t pid,
                                 __WAIT_STATUS stat_loc,
                                 __STDC_INT_AS_UINT_T options,
                                 struct rusage *usage) {
	return libc_wait4(pid, stat_loc, options, usage);
}
/* Same as `waitpid(pid,STAT_LOC,OPTIONS)', though also fills in `USAGE' when non-NULL
 * @param options: Set of `WNOHANG|WUNTRACED|WCONTINUED' (as a KOS extension, `WNOWAIT' is also accepted) */
INTERN ATTR_SECTION(".text.crt.dos.sched.wait") pid_t
NOTHROW_NCX(LIBDCALL libd_wait4_64)(pid_t pid,
                                    __WAIT_STATUS stat_loc,
                                    __STDC_INT_AS_UINT_T options,
                                    struct rusage64 *usage) {
	return libc_wait4_64(pid, stat_loc, options, usage);
}
/* >> detach(2)
...
INTERN ATTR_SECTION(".text.crt.dos.sched.wait") int
NOTHROW_NCX(LIBDCALL libd_detach)(pid_t pid) {
	return libc_detach(pid);
}
...
#endif /* !GUARD_LIBC_AUTO_DOSABI_SYS_WAIT_C */
