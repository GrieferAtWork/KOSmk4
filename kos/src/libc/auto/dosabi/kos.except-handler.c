 ...
 */
#ifndef GUARD_LIBC_AUTO_DOSABI_KOS_EXCEPT_HANDLER_C
#define GUARD_LIBC_AUTO_DOSABI_KOS_EXCEPT_HANDLER_C 1

#include "../../api.h"
#include <hybrid/typecore.h>
#include <kos/types.h>
#include "../../user/kos.except-handler.h"
...
NOTHROW(LIBDCALL libd_set_exception_handler)(unsigned int mode,
                                             except_handler_t handler,
                                             void *handler_sp) {
	return libc_set_exception_handler(mode, handler, handler_sp);
}
...
INTERN ATTR_SECTION(".text.crt.dos.unsorted") int
NOTHROW_NCX(LIBDCALL libd_get_exception_handler)(unsigned int *pmode,
                                                 except_handler_t *phandler,
                                                 void **phandler_sp) {
	return libc_get_exception_handler(pmode, phandler, phandler_sp);
}
/* Mode #2 / #3 exception handler (see description above) */
INTERN ATTR_SECTION(".text.crt.dos.unsorted") ATTR_NORETURN void
(__EXCEPT_HANDLER_CC libd_except_handler3)(error_register_state_t *__restrict state,
                                           struct exception_data *__restrict error) THROWS(...) {
	libc_except_handler3(state, error);
}
/* Mode #4 exception handler (see description above) */
INTERN ATTR_SECTION(".text.crt.dos.unsorted") ATTR_NORETURN void
(__EXCEPT_HANDLER_CC libd_except_handler4)(error_register_state_t *__restrict state,
                                           struct exception_data *__restrict error) THROWS(...) {
	libc_except_handler4(state, error);
}
...
#endif /* !GUARD_LIBC_AUTO_DOSABI_KOS_EXCEPT_HANDLER_C */
