 ...
 */
#ifndef GUARD_LIBC_AUTO_DOSABI_SYS_SENDFILE_C
#define GUARD_LIBC_AUTO_DOSABI_SYS_SENDFILE_C 1

#include "../../api.h"
#include <hybrid/typecore.h>
#include <kos/types.h>
#include "../../user/sys.sendfile.h"
...
INTERN ATTR_SECTION(".text.crt.dos.fs.statfs.statfs") ssize_t
NOTHROW_NCX(LIBDCALL libd_sendfile)(fd_t out_fd,
                                    fd_t in_fd,
                                    off_t *offset,
                                    size_t count) {
 ...
 * Return the number of written bytes, or -1 in case of error */
INTERN ATTR_SECTION(".text.crt.dos.fs.statfs.statfs") ssize_t
NOTHROW_NCX(LIBDCALL libd_sendfile64)(fd_t out_fd,
                                      fd_t in_fd,
                                      off64_t *offset,
                                      size_t count) {
	return libc_sendfile64(out_fd, in_fd, offset, count);
}
...
#endif /* !GUARD_LIBC_AUTO_DOSABI_SYS_SENDFILE_C */
