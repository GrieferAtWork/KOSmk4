 ...
 */
#ifndef GUARD_LIBC_AUTO_DOSABI_FORMAT_PRINTER_C
#define GUARD_LIBC_AUTO_DOSABI_FORMAT_PRINTER_C 1
...
INTERN ATTR_SECTION(".text.crt.dos.string.format") NONNULL((1)) ssize_t
(LIBDCALL libd_format_repeat)(pformatprinter printer,
                              void *arg,
                              char ch,
                              size_t num_repetitions) THROWS(...) {
	return libc_format_repeat(printer, arg, ch, num_repetitions);
}
...
INTERN ATTR_SECTION(".text.crt.dos.string.format") NONNULL((1)) ssize_t
(LIBDCALL libd_format_escape)(pformatprinter printer,
                              void *arg,
                              char const *__restrict text,
                              size_t textlen,
                              unsigned int flags) THROWS(...) {
	return libc_format_escape(printer, arg, text, textlen, flags);
}
...
INTERN ATTR_SECTION(".text.crt.dos.string.format") NONNULL((1)) ssize_t
(LIBDCALL libd_format_hexdump)(pformatprinter printer,
                               void *arg,
                               void const *__restrict data,
                               size_t size,
                               size_t linesize,
                               unsigned int flags) THROWS(...) {
	return libc_format_hexdump(printer, arg, data, size, linesize, flags);
}
...
INTERN ATTR_SECTION(".text.crt.dos.string.format") ATTR_LIBC_PRINTF(3, 0) NONNULL((1, 3)) ssize_t
(LIBDCALL libd_format_vprintf)(pformatprinter printer,
                               void *arg,
                               char const *__restrict format,
                               va_list args) THROWS(...) {
	return libc_format_vprintf(printer, arg, format, args);
}
/* Generic printf implementation
 * Taking a regular printf-style format string and arguments, these
 * functions will call the given `PRINTER' callback with various strings
...
INTERN ATTR_SECTION(".text.crt.dos.string.format") ATTR_LIBC_SCANF(4, 0) NONNULL((1, 2, 4)) ssize_t
(LIBDCALL libd_format_vscanf)(pformatgetc pgetc,
                              pformatungetc pungetc,
                              void *arg,
                              char const *__restrict format,
                              va_list args) THROWS(...) {
	return libc_format_vscanf(pgetc, pungetc, arg, format, args);
}
/* Generic scanf implementation
 * Taking a regular scanf-style format string and argument, these
 * functions will call the given `SCANNER' function which in
...
INTERN ATTR_SECTION(".text.crt.dos.string.format") NONNULL((1, 2)) ssize_t
NOTHROW_NCX(LIBDCALL libd_format_sprintf_printer)(void *arg,
                                                  char const *__restrict data,
                                                  size_t datalen) {
	return libc_format_sprintf_printer(arg, data, datalen);
}
...
INTERN ATTR_SECTION(".text.crt.dos.string.format") NONNULL((1, 2)) ssize_t
NOTHROW_NCX(LIBDCALL libd_format_snprintf_printer)(void *arg,
                                                   char const *__restrict data,
                                                   size_t datalen) {
	return libc_format_snprintf_printer(arg, data, datalen);
}
/* Returns the width (number of characters; not bytes) of the given unicode string */
INTERN ATTR_SECTION(".text.crt.dos.string.format") ATTR_PURE NONNULL((2)) ssize_t
NOTHROW_NCX(LIBDCALL libd_format_width)(void *arg,
                                        char const *__restrict data,
                                        size_t datalen) {
	return libc_format_width(arg, data, datalen);
}
/* Always re-return `datalen' and ignore all other arguments */
INTERN ATTR_SECTION(".text.crt.dos.string.format") ATTR_CONST ssize_t
NOTHROW_NCX(LIBDCALL libd_format_length)(void *arg,
                                         char const *__restrict data,
                                         size_t datalen) {
	return libc_format_length(arg, data, datalen);
}
...
INTERN ATTR_SECTION(".text.crt.dos.string.format") ATTR_MALLOC ATTR_MALL_DEFAULT_ALIGNED WUNUSED NONNULL((1)) char *
NOTHROW_NCX(LIBDCALL libd_format_aprintf_pack)(struct format_aprintf_data *__restrict self,
                                               size_t *pstrlen) {
	return libc_format_aprintf_pack(self, pstrlen);
}
...
INTERN ATTR_SECTION(".text.crt.dos.string.format") ATTR_MALLOC ATTR_MALL_DEFAULT_ALIGNED WUNUSED ATTR_ALLOC_SIZE((2)) NONNULL((1)) char *
NOTHROW_NCX(LIBDCALL libd_format_aprintf_alloc)(struct format_aprintf_data *__restrict self,
                                                size_t num_chars) {
	return libc_format_aprintf_alloc(self, num_chars);
}
/* Print data to a dynamically allocated heap buffer. On error, -1 is returned
 * This function is intended to be used as a pformatprinter-compatibile printer sink */
INTERN ATTR_SECTION(".text.crt.dos.string.format") WUNUSED NONNULL((1, 2)) ssize_t
NOTHROW_NCX(LIBDCALL libd_format_aprintf_printer)(void *arg,
                                                  char const *__restrict data,
                                                  size_t datalen) {
	return libc_format_aprintf_printer(arg, data, datalen);
}
...
#endif /* !GUARD_LIBC_AUTO_DOSABI_FORMAT_PRINTER_C */
