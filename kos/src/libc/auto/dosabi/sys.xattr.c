/* HASH CRC-32:0xab1810fa */
/* Copyright (c) 2019-2020 Griefer@Work                                       *
 *                                                                            *
 * This software is provided 'as-is', without any express or implied          *
 * warranty. In no event will the authors be held liable for any damages      *
 * arising from the use of this software.                                     *
 *                                                                            *
 * Permission is granted to anyone to use this software for any purpose,      *
 * including commercial applications, and to alter it and redistribute it     *
 * freely, subject to the following restrictions:                             *
 *                                                                            *
 * 1. The origin of this software must not be misrepresented; you must not    *
 *    claim that you wrote the original software. If you use this software    *
 *    in a product, an acknowledgement (see the following) in the product     *
 *    documentation is required:                                              *
 *    Portions Copyright (c) 2019-2020 Griefer@Work                           *
 * 2. Altered source versions must be plainly marked as such, and must not be *
 *    misrepresented as being the original software.                          *
 * 3. This notice may not be removed or altered from any source distribution. *
 */
#ifndef GUARD_LIBC_AUTO_DOSABI_SYS_XATTR_C
#define GUARD_LIBC_AUTO_DOSABI_SYS_XATTR_C 1

#include "../../api.h"
#include <hybrid/typecore.h>
#include <kos/types.h>
#include "../../user/sys.xattr.h"

DECL_BEGIN

/* Set the attribute NAME of the file pointed to by PATH to VALUE
 * (which is SIZE bytes long). Return 0 on success, -1 for errors
 * @param: flags: 0, or a one of `XATTR_*' */
INTERN ATTR_SECTION(".text.crt.dos.unsorted") NONNULL((1, 2, 3)) int
NOTHROW_RPC(LIBDCALL libd_setxattr)(char const *path,
                                    char const *name,
                                    void const *buf,
                                    size_t bufsize,
                                    __STDC_INT_AS_UINT_T flags) {
	return libc_setxattr(path, name, buf, bufsize, flags);
}
/* Set the attribute NAME of the file pointed to by PATH to VALUE (which is
 * SIZE bytes long), not following symlinks for the last pathname component.
 * Return 0 on success, -1 for errors
 * @param: flags: 0, or a one of `XATTR_*' */
INTERN ATTR_SECTION(".text.crt.dos.unsorted") NONNULL((1, 2, 3)) int
NOTHROW_RPC(LIBDCALL libd_lsetxattr)(char const *path,
                                     char const *name,
                                     void const *buf,
                                     size_t bufsize,
                                     __STDC_INT_AS_UINT_T flags) {
	return libc_lsetxattr(path, name, buf, bufsize, flags);
}
/* Set the attribute NAME of the file descriptor FD to VALUE
 * (which is SIZE bytes long). Return 0 on success, -1 for errors
 * @param: flags: 0, or a one of `XATTR_*' */
INTERN ATTR_SECTION(".text.crt.dos.unsorted") NONNULL((2, 3)) int
NOTHROW_RPC(LIBDCALL libd_fsetxattr)(fd_t fd,
                                     char const *name,
                                     void const *buf,
                                     size_t bufsize,
                                     __STDC_INT_AS_UINT_T flags) {
	return libc_fsetxattr(fd, name, buf, bufsize, flags);
}
/* Get the attribute NAME of the file pointed to by PATH to VALUE
 * (which is SIZE bytes long). Return 0 on success, -1 for errors */
INTERN ATTR_SECTION(".text.crt.dos.unsorted") NONNULL((1, 2, 3)) ssize_t
NOTHROW_RPC(LIBDCALL libd_getxattr)(char const *path,
                                    char const *name,
                                    void *buf,
                                    size_t bufsize) {
	return libc_getxattr(path, name, buf, bufsize);
}
/* Get the attribute NAME of the file pointed to by PATH to VALUE (which is
 * SIZE bytes long), not following symlinks for the last pathname component.
 * Return 0 on success, -1 for errors */
INTERN ATTR_SECTION(".text.crt.dos.unsorted") NONNULL((1, 2, 3)) ssize_t
NOTHROW_RPC(LIBDCALL libd_lgetxattr)(char const *path,
                                     char const *name,
                                     void *buf,
                                     size_t bufsize) {
	return libc_lgetxattr(path, name, buf, bufsize);
}
/* Get the attribute NAME of the file descriptor FD to VALUE
 * (which is SIZE bytes long). Return 0 on success, -1 for errors */
INTERN ATTR_SECTION(".text.crt.dos.unsorted") NONNULL((2, 3)) ssize_t
NOTHROW_RPC(LIBDCALL libd_fgetxattr)(fd_t fd,
                                     char const *name,
                                     void *buf,
                                     size_t bufsize) {
	return libc_fgetxattr(fd, name, buf, bufsize);
}
/* List attributes of the file pointed to by PATH into the
 * user-supplied buffer LIST (which is SIZE bytes big).
 * Return 0 on success, -1 for errors */
INTERN ATTR_SECTION(".text.crt.dos.unsorted") NONNULL((1, 2)) ssize_t
NOTHROW_RPC(LIBDCALL libd_listxattr)(char const *path,
                                     char *listbuf,
                                     size_t listbufsize) {
	return libc_listxattr(path, listbuf, listbufsize);
}
/* List attributes of the file pointed to by PATH into the user-supplied
 * buffer LIST (which is SIZE bytes big), not following symlinks for the
 * last pathname component. Return 0 on success, -1 for errors */
INTERN ATTR_SECTION(".text.crt.dos.unsorted") NONNULL((1, 2)) ssize_t
NOTHROW_RPC(LIBDCALL libd_llistxattr)(char const *path,
                                      char *listbuf,
                                      size_t listbufsize) {
	return libc_llistxattr(path, listbuf, listbufsize);
}
/* List attributes of the file descriptor FD into the user-supplied buffer
 * LIST (which is SIZE bytes big). Return 0 on success, -1 for errors */
INTERN ATTR_SECTION(".text.crt.dos.unsorted") NONNULL((2)) ssize_t
NOTHROW_RPC(LIBDCALL libd_flistxattr)(fd_t fd,
                                      char *listbuf,
                                      size_t listbufsize) {
	return libc_flistxattr(fd, listbuf, listbufsize);
}
/* Remove the attribute NAME from the file pointed to by PATH.
 * Return 0 on success, -1 for errors */
INTERN ATTR_SECTION(".text.crt.dos.unsorted") NONNULL((1, 2)) int
NOTHROW_RPC(LIBDCALL libd_removexattr)(char const *path,
                                       char const *name) {
	return libc_removexattr(path, name);
}
/* Remove the attribute NAME from the file pointed to by PATH, not
 * following symlinks for the last pathname component.
 * Return 0 on success, -1 for errors */
INTERN ATTR_SECTION(".text.crt.dos.unsorted") NONNULL((1, 2)) int
NOTHROW_RPC(LIBDCALL libd_lremovexattr)(char const *path,
                                        char const *name) {
	return libc_lremovexattr(path, name);
}
/* Remove the attribute NAME from the file descriptor FD.
 * Return 0 on success, -1 for errors */
INTERN ATTR_SECTION(".text.crt.dos.unsorted") NONNULL((2)) int
NOTHROW_RPC(LIBDCALL libd_fremovexattr)(fd_t fd,
                                        char const *name) {
	return libc_fremovexattr(fd, name);
}

DECL_END

DEFINE_PUBLIC_ALIAS(DOS$setxattr, libd_setxattr);
DEFINE_PUBLIC_ALIAS(DOS$lsetxattr, libd_lsetxattr);
DEFINE_PUBLIC_ALIAS(DOS$fsetxattr, libd_fsetxattr);
DEFINE_PUBLIC_ALIAS(DOS$getxattr, libd_getxattr);
DEFINE_PUBLIC_ALIAS(DOS$lgetxattr, libd_lgetxattr);
DEFINE_PUBLIC_ALIAS(DOS$fgetxattr, libd_fgetxattr);
DEFINE_PUBLIC_ALIAS(DOS$listxattr, libd_listxattr);
DEFINE_PUBLIC_ALIAS(DOS$llistxattr, libd_llistxattr);
DEFINE_PUBLIC_ALIAS(DOS$flistxattr, libd_flistxattr);
DEFINE_PUBLIC_ALIAS(DOS$removexattr, libd_removexattr);
DEFINE_PUBLIC_ALIAS(DOS$lremovexattr, libd_lremovexattr);
DEFINE_PUBLIC_ALIAS(DOS$fremovexattr, libd_fremovexattr);

#endif /* !GUARD_LIBC_AUTO_DOSABI_SYS_XATTR_C */
