 ...
 */
#ifndef GUARD_LIBC_AUTO_DOSABI_KOS_FUTEXEXPR_C
#define GUARD_LIBC_AUTO_DOSABI_KOS_FUTEXEXPR_C 1
...
INTERN ATTR_SECTION(".text.crt.dos.sched.futexexpr") NONNULL((3)) int
NOTHROW_RPC(LIBDCALL libd_lfutexexpr)(void *base,
                                      size_t exprc,
                                      struct lfutexexpr const *exprv,
                                      struct timespec const *timeout,
                                      unsigned int timeout_flags) {
	return libc_lfutexexpr(base, exprc, exprv, timeout, timeout_flags);
}
/* >> lfutexlockexpr(2)
 ...
 * @return: -1:EINTR:     A blocking futex-wait operation was interrupted
 * @return: -1:ETIMEDOUT: A blocking futex-wait operation has timed out */
INTERN ATTR_SECTION(".text.crt.dos.sched.futexlockexpr") NONNULL((1, 4)) int
NOTHROW_RPC(LIBDCALL libd_lfutexlockexpr)(lfutex_t *ulockaddr,
                                          void *base,
                                          size_t exprc,
                                          struct lfutexexpr const *exprv,
                                          struct timespec const *timeout,
                                          unsigned int timeout_flags) {
	return libc_lfutexlockexpr(ulockaddr, base, exprc, exprv, timeout, timeout_flags);
}
/* >> lfutexexpr(2)
 * The lfutexexpr() system call can be used to specify arbitrarily complex
 * expressions that must atomically (in relation to other futex operations)
 ...
 * @return: -1:ETIMEDOUT: A blocking futex-wait operation has timed out */
INTERN ATTR_SECTION(".text.crt.dos.sched.futexexpr") NONNULL((3)) int
NOTHROW_RPC(LIBDCALL libd_lfutexexpr64)(void *base,
                                        size_t exprc,
                                        struct lfutexexpr const *exprv,
                                        struct timespec64 const *timeout,
                                        unsigned int timeout_flags) {
	return libc_lfutexexpr64(base, exprc, exprv, timeout, timeout_flags);
}
/* >> lfutexlockexpr(2)
 * A function that is similar to `lfutexexpr()', but allows for the use of one central
 * locking futex that is used for waiting and may be distinct from any other given futex
 ...
 * @return: -1:ETIMEDOUT: A blocking futex-wait operation has timed out */
INTERN ATTR_SECTION(".text.crt.dos.sched.futexlockexpr") NONNULL((1, 4)) int
NOTHROW_RPC(LIBDCALL libd_lfutexlockexpr64)(lfutex_t *ulockaddr,
                                            void *base,
                                            size_t exprc,
                                            struct lfutexexpr const *exprv,
                                            struct timespec64 const *timeout,
                                            unsigned int timeout_flags) {
	return libc_lfutexlockexpr64(ulockaddr, base, exprc, exprv, timeout, timeout_flags);
}
...
#endif /* !GUARD_LIBC_AUTO_DOSABI_KOS_FUTEXEXPR_C */
