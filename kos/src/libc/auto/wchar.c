...
NOTHROW_NCX(LIBDCALL libd__wcslwr_l)(char16_t *__restrict str,
	...
	for (iter = str; (ch = *iter) != '\0'; ++iter)
		*iter = (char16_t)towlower_l(ch, locale);
	return str;
}
...
NOTHROW_NCX(LIBKCALL libc_wcslwr_l)(char32_t *__restrict str,
	...
	char32_t *iter, ch;
	for (iter = str; (ch = *iter) != '\0'; ++iter)
		*iter = (char32_t)towlower_l(ch, locale);
	return str;
}
INTERN ATTR_SECTION(".text.crt.dos.wchar.unicode.locale.memory") ATTR_LEAF ATTR_RETNONNULL NONNULL((1)) char16_t *
NOTHROW_NCX(LIBDCALL libd__wcsupr_l)(char16_t *__restrict str,
	...
	char16_t *iter, ch;
	for (iter = str; (ch = *iter) != '\0'; ++iter)
		*iter = (char16_t)towupper_l(ch, locale);
	return str;
}
INTERN ATTR_SECTION(".text.crt.wchar.unicode.locale.memory") ATTR_LEAF ATTR_RETNONNULL NONNULL((1)) char32_t *
NOTHROW_NCX(LIBKCALL libc_wcsupr_l)(char32_t *__restrict str,
	...
	char32_t *iter, ch;
	for (iter = str; (ch = *iter) != '\0'; ++iter)
		*iter = (char32_t)towupper_l(ch, locale);
	return str;
}
INTERN ATTR_SECTION(".text.crt.dos.wchar.unicode.locale.memory") ATTR_LEAF ATTR_RETNONNULL NONNULL((1)) char16_t *
...
NOTHROW_NCX(LIBDCALL libd_fuzzy_wmemcasecmp_l)(char16_t const *s1,
	...
	for (i = 0; i < s1_bytes; ++i) {
		...
		for (j = 0; j < s2_bytes; j++) {
			byte_t c1 = ((byte_t *)s1)[i];
			byte_t c2 = ((byte_t *)s2)[j];
			cost  = c1 != c2 && (char16_t)towlower_l(c1, locale) != (char16_t)towlower_l(c2, locale);
			cost += v0[j];
			temp  = v1[j] + 1;
			if (cost > temp)
				cost = temp;
...
NOTHROW_NCX(LIBKCALL libc_fuzzy_wmemcasecmp_l)(char32_t const *s1,
	...
	for (i = 0; i < s1_bytes; ++i) {
		...
		for (j = 0; j < s2_bytes; j++) {
			byte_t c1 = ((byte_t *)s1)[i];
			byte_t c2 = ((byte_t *)s2)[j];
			cost  = c1 != c2 && (char32_t)towlower_l(c1, locale) != (char32_t)towlower_l(c2, locale);
			cost += v0[j];
			temp  = v1[j] + 1;
			if (cost > temp)
				cost = temp;
...
NOTHROW_NCX(LIBDCALL libd_wildwcscasecmp_l)(char16_t const *pattern,
	...
	for (;;) {
		...
		if (*pattern == '*') {
			...
			if (card_post == '?')
				goto next; /* Match any --> already found */
			card_post = (char16_t)towlower_l(card_post, locale);
			for (;;) {
				char16_t ch = *string++;
				if (card_post == ch || card_post == (char16_t)towlower_l(ch, locale)) {
					/* Recursively check if the rest of the string and pattern match */
		...
		wcsing_ch = *string;
		if (pattern_ch == wcsing_ch || pattern_ch == '?' ||
		   (pattern_ch = (char16_t)towlower_l(pattern_ch, locale),
		    wcsing_ch = (char16_t)towlower_l(wcsing_ch, locale),
		    pattern_ch == wcsing_ch)) {
next:
			++string;
			++pattern;
...
NOTHROW_NCX(LIBKCALL libc_wildwcscasecmp_l)(char32_t const *pattern,
	...
	for (;;) {
		...
		if (*pattern == '*') {
			...
			if (card_post == '?')
				goto next; /* Match any --> already found */
			card_post = (char32_t)towlower_l(card_post, locale);
			for (;;) {
				char32_t ch = *string++;
				if (card_post == ch || card_post == (char32_t)towlower_l(ch, locale)) {
					/* Recursively check if the rest of the string and pattern match */
		...
		wcsing_ch = *string;
		if (pattern_ch == wcsing_ch || pattern_ch == '?' ||
		   (pattern_ch = (char32_t)towlower_l(pattern_ch, locale),
		    wcsing_ch = (char32_t)towlower_l(wcsing_ch, locale),
		    pattern_ch == wcsing_ch)) {
next:
			++string;
			++pattern;
...
DEFINE_PUBLIC_ALIAS(_swprintf, libc_swprintf);
...
DEFINE_PUBLIC_ALIAS(_wcsicmp, libc_wcscasecmp);
DEFINE_PUBLIC_ALIAS(wcsicmp, libc_wcscasecmp);
DEFINE_PUBLIC_ALIAS(wcscasecmp, libc_wcscasecmp);
DEFINE_PUBLIC_ALIAS(DOS$wcsncasecmp, libd_wcsncasecmp);
DEFINE_PUBLIC_ALIAS(_wcsnicmp, libc_wcsncasecmp);
DEFINE_PUBLIC_ALIAS(wcsnicmp, libc_wcsncasecmp);
DEFINE_PUBLIC_ALIAS(wcsncasecmp, libc_wcsncasecmp);
DEFINE_PUBLIC_ALIAS(DOS$wcscasecmp_l, libd_wcscasecmp_l);
DEFINE_PUBLIC_ALIAS(_wcsicmp_l, libc_wcscasecmp_l);
DEFINE_PUBLIC_ALIAS(wcsicmp_l, libc_wcscasecmp_l);
...
DEFINE_PUBLIC_ALIAS(_wcsnicmp_l, libc_wcsncasecmp_l);
DEFINE_PUBLIC_ALIAS(wcsnicmp_l, libc_wcsncasecmp_l);
...
DEFINE_PUBLIC_ALIAS(_wcscoll_l, libc_wcscoll_l);
...
DEFINE_PUBLIC_ALIAS(_wcsxfrm_l, libc_wcsxfrm_l);
...
DEFINE_PUBLIC_ALIAS(_wcstol_l, libc_wcstol_l);
...
DEFINE_PUBLIC_ALIAS(_wcstoul_l, libc_wcstoul_l);
...
DEFINE_PUBLIC_ALIAS(_wcstof_l, libc_wcstof_l);
...
DEFINE_PUBLIC_ALIAS(_wcstod_l, libc_wcstod_l);
...
DEFINE_PUBLIC_ALIAS(_wcstold_l, libc_wcstold_l);
