...
INTDEF ATTR_RETNONNULL ATTR_ACCESS_RW(3) NONNULL((1)) char *NOTHROW_NCX(LIBDCALL libd_bgets)(char *buf, size_t buflen_minus_one, FILE *fp, char __KOS_FIXED_CONST *stop_chars);
#endif /* !__LIBCCALL_IS_LIBDCALL && !__KERNEL__ */
#ifndef __KERNEL__
/* >> bgets(3)
 * Read at most `buflen_minus_one' characters from `fp' and into `buf'
 ...
 *                There is no true error  case; read errors are  treated
 *                the same way as end-of-file. */
INTDEF ATTR_RETNONNULL ATTR_ACCESS_RW(3) NONNULL((1)) char *NOTHROW_NCX(LIBCCALL libc_bgets)(char *buf, size_t buflen_minus_one, FILE *fp, char __KOS_FIXED_CONST *stop_chars);
#endif /* !__KERNEL__ */
#if !defined(__LIBCCALL_IS_LIBDCALL) && !defined(__KERNEL__)
/* >> bufsplit(3)
...
INTDEF size_t NOTHROW_NCX(LIBDCALL libd_bufsplit)(char *string, size_t result_c, char **result_v);
#endif /* !__LIBCCALL_IS_LIBDCALL && !__KERNEL__ */
#ifndef __KERNEL__
/* >> bufsplit(3)
 * Split `string' on field-separator characters (by default "\t" or  "\n"),
 ...
 *              beyond `return' (and  before `result_c')  are filled  with
 *              the empty string found at `strend(IN:string)'. */
INTDEF size_t NOTHROW_NCX(LIBCCALL libc_bufsplit)(char *string, size_t result_c, char **result_v);
INTDEF NONNULL((2)) char *NOTHROW_NCX(LIBCCALL libc_fcopylist_sz)(fd_t fd, size_t *p_filesize);
INTDEF NONNULL((1, 2)) char *NOTHROW_NCX(LIBDCALL libd_copylist_sz)(char const *filename, size_t *p_filesize);
INTDEF NONNULL((1, 2)) char *NOTHROW_NCX(LIBCCALL libc_copylist_sz)(char const *filename, size_t *p_filesize);
 ...
 *         this architecture. */
INTDEF WUNUSED NONNULL((1, 2)) char *NOTHROW_RPC(LIBDCALL libd_copylist)(char const *filename, __PIO_OFFSET *p_filesize);
/* >> copylist(3), copylist64(3)
 * Load a given file `filename' into memory (via `malloc(3)'), and return
 * a  pointer to this newly malloc'd data-blob (the caller is responsible
 * to eventually free(3) it). The size of this loaded data blob is stored
 ...
 *         64-bit, meaning that its existence isn't explicitly warranted on
 *         this architecture. */
INTDEF WUNUSED NONNULL((1, 2)) char *NOTHROW_RPC(LIBCCALL libc_copylist)(char const *filename, __PIO_OFFSET *p_filesize);
/* >> copylist(3), copylist64(3)
 * Load a given file `filename' into memory (via `malloc(3)'), and return
 * a  pointer to this newly malloc'd data-blob (the caller is responsible
 * to eventually free(3) it). The size of this loaded data blob is stored
 ...
 *         64-bit, meaning that its existence isn't explicitly warranted on
 *         this architecture. */
INTDEF WUNUSED NONNULL((1, 2)) char *NOTHROW_RPC(LIBDCALL libd_copylist64)(char const *filename, __PIO_OFFSET64 *p_filesize);
/* >> copylist(3), copylist64(3)
 * Load a given file `filename' into memory (via `malloc(3)'), and return
 * a  pointer to this newly malloc'd data-blob (the caller is responsible
 * to eventually free(3) it). The size of this loaded data blob is stored
 ...
 *         64-bit, meaning that its existence isn't explicitly warranted on
 *         this architecture. */
INTDEF WUNUSED NONNULL((1, 2)) char *NOTHROW_RPC(LIBCCALL libc_copylist64)(char const *filename, __PIO_OFFSET64 *p_filesize);
#endif /* !__KERNEL__ */
#if !defined(__LIBCCALL_IS_LIBDCALL) && !defined(__KERNEL__)
/* >> strcadd(3)
...
INTDEF ATTR_RETNONNULL NONNULL((1, 2)) char *NOTHROW_NCX(LIBDCALL libd_strcadd)(char *dstbuf, char const *string);
#endif /* !__LIBCCALL_IS_LIBDCALL && !__KERNEL__ */
#ifndef __KERNEL__
/* >> strcadd(3)
 * Decode C-style escape sequences from `string' and write the result to `dstbuf'
 ...
 * bytes long (the +1 because this function appends a trailing '\0')
 * @return: * : A pointer to the trailing '\0' appended to `dstbuf' */
INTDEF ATTR_RETNONNULL NONNULL((1, 2)) char *NOTHROW_NCX(LIBCCALL libc_strcadd)(char *dstbuf, char const *string);
#endif /* !__KERNEL__ */
#if !defined(__LIBCCALL_IS_LIBDCALL) && !defined(__KERNEL__)
/* >> strccpy(3)
 * Same as `strcadd()', but re-returns `dstbuf' rather than `strend(dstbuf)'
 * @return: dstbuf: Always re-returns `dstbuf' */
INTDEF ATTR_RETNONNULL NONNULL((1, 2)) char *NOTHROW_NCX(LIBDCALL libd_strccpy)(char *dstbuf, char const *string);
#endif /* !__LIBCCALL_IS_LIBDCALL && !__KERNEL__ */
#ifndef __KERNEL__
/* >> strccpy(3)
 * Same as `strcadd()', but re-returns `dstbuf' rather than `strend(dstbuf)'
 * @return: dstbuf: Always re-returns `dstbuf' */
INTDEF ATTR_RETNONNULL NONNULL((1, 2)) char *NOTHROW_NCX(LIBCCALL libc_strccpy)(char *dstbuf, char const *string);
#endif /* !__KERNEL__ */
#if !defined(__LIBCCALL_IS_LIBDCALL) && !defined(__KERNEL__)
/* >> strcadd(3)
 ...
 * @return: * : A pointer to the trailing '\0' appended to `dstbuf' */
INTDEF ATTR_RETNONNULL NONNULL((1, 2)) char *NOTHROW_NCX(LIBDCALL libd_streadd)(char *dstbuf, char const *string, char const *dont_encode);
#endif /* !__LIBCCALL_IS_LIBDCALL && !__KERNEL__ */
#ifndef __KERNEL__
/* >> strcadd(3)
 * Encode a given `string' using C-style escape sequences, and write the result to `dstbuf'
 ...
 * When non-NULL, characters from `dont_encode' are not encoded, but instead kept as-is.
 * @return: * : A pointer to the trailing '\0' appended to `dstbuf' */
INTDEF ATTR_RETNONNULL NONNULL((1, 2)) char *NOTHROW_NCX(LIBCCALL libc_streadd)(char *dstbuf, char const *string, char const *dont_encode);
#endif /* !__KERNEL__ */
#if !defined(__LIBCCALL_IS_LIBDCALL) && !defined(__KERNEL__)
/* >> strecpy(3)
 * Same as `streadd()', but re-returns `dstbuf' rather than `strend(dstbuf)'
 * @return: dstbuf: Always re-returns `dstbuf' */
INTDEF ATTR_RETNONNULL NONNULL((1, 2)) char *NOTHROW_NCX(LIBDCALL libd_strecpy)(char *dstbuf, char const *string, char const *dont_encode);
#endif /* !__LIBCCALL_IS_LIBDCALL && !__KERNEL__ */
#ifndef __KERNEL__
/* >> strecpy(3)
 * Same as `streadd()', but re-returns `dstbuf' rather than `strend(dstbuf)'
 * @return: dstbuf: Always re-returns `dstbuf' */
INTDEF ATTR_RETNONNULL NONNULL((1, 2)) char *NOTHROW_NCX(LIBCCALL libc_strecpy)(char *dstbuf, char const *string, char const *dont_encode);
#endif /* !__KERNEL__ */
#if !defined(__LIBCCALL_IS_LIBDCALL) && !defined(__KERNEL__)
/* >> strfind(3)
...
INTDEF ATTR_PURE WUNUSED NONNULL((1, 2)) __STDC_INT_AS_SSIZE_T NOTHROW_NCX(LIBDCALL libd_strfind)(char const *haystack, char const *needle);
#endif /* !__LIBCCALL_IS_LIBDCALL && !__KERNEL__ */
#ifndef __KERNEL__
/* >> strfind(3)
 * Same as `p = strstr(haystack, needle); p ? p - haystack : -1'
 * @return: * : `needle' found at `haystack + return'
 * @return: -1: `needle' not found in `haystack' */
INTDEF ATTR_PURE WUNUSED NONNULL((1, 2)) __STDC_INT_AS_SSIZE_T NOTHROW_NCX(LIBCCALL libc_strfind)(char const *haystack, char const *needle);
#endif /* !__KERNEL__ */
#if !defined(__LIBCCALL_IS_LIBDCALL) && !defined(__KERNEL__)
/* >> strrspn(3)
...
INTDEF ATTR_PURE ATTR_RETNONNULL WUNUSED NONNULL((1, 2)) char *NOTHROW_NCX(LIBDCALL libd_strrspn)(char const *haystack, char const *accept);
#endif /* !__LIBCCALL_IS_LIBDCALL && !__KERNEL__ */
#ifndef __KERNEL__
/* >> strrspn(3)
 * Scanning from the right, return a pointer to last character in  `haystack'
 * for which `strchr(accept, ch) != NULL'. If `haystack' consists entirely of
 * characters from `accept', re-returns `haystack'. */
INTDEF ATTR_PURE ATTR_RETNONNULL WUNUSED NONNULL((1, 2)) char *NOTHROW_NCX(LIBCCALL libc_strrspn)(char const *haystack, char const *accept);
#endif /* !__KERNEL__ */
#if !defined(__LIBCCALL_IS_LIBDCALL) && !defined(__KERNEL__)
/* >> strtrns(3)
...
INTDEF ATTR_RETNONNULL NONNULL((1, 2, 3, 4)) char *NOTHROW_NCX(LIBDCALL libd_strtrns)(char const *string, char const *find_map, char const *repl_map, char *result);
#endif /* !__LIBCCALL_IS_LIBDCALL && !__KERNEL__ */
#ifndef __KERNEL__
/* >> strtrns(3)
 * Copy `string' to `result'  (including its trailing NUL),  but
 ...
 * has space for at least `strlen(string) + 1' characters.
 * @return: result: Always re-returns `result' */
INTDEF ATTR_RETNONNULL NONNULL((1, 2, 3, 4)) char *NOTHROW_NCX(LIBCCALL libc_strtrns)(char const *string, char const *find_map, char const *repl_map, char *result);
#endif /* !__KERNEL__ */
