 ...
 * @return: -1:       Error (s.a. `errno') */
INTDEF ATTR_OUTS(2, 3) __STDC_INT_AS_SSIZE_T NOTHROW_RPC(LIBDCALL libd_epoll_wait)(fd_t epfd, struct epoll_event *events, __STDC_INT_AS_SIZE_T maxevents, int timeout);
/* >> epoll_pwait(2)
 ...
 * @return: 0:        No events happened before `timeout' expired.
 * @return: -1:       Error (s.a. `errno') */
INTDEF ATTR_OUTS(2, 3) __STDC_INT_AS_SSIZE_T NOTHROW_RPC(LIBDCALL libd_epoll_pwait)(fd_t epfd, struct epoll_event *events, __STDC_INT_AS_SIZE_T maxevents, int timeout, sigset_t const *ss);
/* >> epoll_rpc_exec(3)
...
INTDEF ATTR_IN(3) NONNULL((6)) int NOTHROW_NCX(LIBDCALL libd_epoll_rpc_exec)(fd_t epfd, fd_t fd, struct epoll_event const *event, pid_t target_tid, unsigned int mode, prpc_exec_callback_t func);
