...
INTDEF ATTR_ACCESS_WR(1) int NOTHROW_NCX(LIBDCALL libd_sem_init)(sem_t *self, int pshared, unsigned int value);
/* >> sem_destroy(3)
 * Destroy a semaphore previously initialized by `sem_init(3)'
 * @return: 0: Success */
INTDEF ATTR_ACCESS_RW(1) int NOTHROW_NCX(LIBDCALL libd_sem_destroy)(sem_t *self);
/* >> sem_open(3)
...
INTDEF ATTR_ACCESS_RO(1) sem_t *NOTHROW_RPC_KOS(VLIBDCALL libd_sem_open)(char const *name, oflag_t oflags, ...);
/* >> sem_close(3)
 ...
 * @return: 0: Success */
INTDEF ATTR_ACCESS_RW(1) int NOTHROW_NCX(LIBDCALL libd_sem_close)(sem_t *self);
/* >> sem_unlink(3)
...
INTDEF ATTR_ACCESS_RO(1) int NOTHROW_RPC_KOS(LIBDCALL libd_sem_unlink)(const char *name);
/* >> sem_wait(3)
...
INTDEF ATTR_ACCESS_RO(2) ATTR_ACCESS_RW(1) int NOTHROW_RPC(LIBDCALL libd_sem_timedwait)(sem_t *__restrict self, struct timespec const *__restrict abstime);
/* >> sem_timedwait(3), sem_timedwait64(3)
 * Wait  for a  ticket to become  available to the  given semaphore `self'
 * Once a ticket has become available, consume it and return. If no ticket
 * becomes  available until `abstime' has passed, return `errno=ETIMEDOUT'
 ...
 * @return: -1: [errno=EINTR]     Interrupted.
 * @return: -1: [errno=ETIMEDOUT] The given `abstime' expired before a ticket became available. */
INTDEF ATTR_ACCESS_RO(2) ATTR_ACCESS_RW(1) int NOTHROW_RPC(LIBDCALL libd_sem_timedwait64)(sem_t *__restrict self, struct timespec64 const *__restrict abstime);
/* >> sem_trywait(3)
...
INTDEF ATTR_ACCESS_RW(1) int NOTHROW_NCX(LIBDCALL libd_sem_trywait)(sem_t *self);
/* >> sem_post(3)
...
INTDEF ATTR_ACCESS_RW(1) int NOTHROW_NCX(LIBDCALL libd_sem_post)(sem_t *self);
/* >> sem_getvalue(3)
 * Capture a snapshot of how may tickets are available storing that number in `*sval'
 * @return: 0: Success */
INTDEF ATTR_ACCESS_RW(1) ATTR_ACCESS_WR(2) int NOTHROW_NCX(LIBDCALL libd_sem_getvalue)(sem_t *__restrict self, __STDC_INT_AS_UINT_T *__restrict sval);
