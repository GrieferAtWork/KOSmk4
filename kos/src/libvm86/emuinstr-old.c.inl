...
(CC libvm86_pushw)(vm86_state_t *__restrict self,
                   uint16_t value) {
	uint16_t *addr;
	if unlikely(!vm86_state_canpush(self, 2))
...
(CC libvm86_pushl)(vm86_state_t *__restrict self,
                   uint32_t value) {
	uint32_t *addr;
	if unlikely(!vm86_state_canpush(self, 4))
		return VM86_DOUBLE_FAULT;
...
(CC libvm86_popw)(vm86_state_t *__restrict self,
                  uint16_t *__restrict presult) {
	uint16_t *addr, value;
	if unlikely(!vm86_state_canpop(self, 2))
		return VM86_DOUBLE_FAULT;
...
(CC libvm86_popl)(vm86_state_t *__restrict self,
                  uint32_t *__restrict presult) {
	uint32_t *addr, value;
	if unlikely(!vm86_state_canpop(self, 4))
		return VM86_DOUBLE_FAULT;
...
libvm86_modrm_getaddr(vm86_state_t *__restrict self,
                      ...
                      emu86_opflags_t op_flags) {
	uintptr_t addr;
	if (EMU86_MODRM_ISREG(info->mi_type))
...
#define DEFINE_LXS(opcode, segment_register)                          \
	case opcode: {                                                    \
		...
		if unlikely(error != VM86_SUCCESS)                            \
			goto err;                                                 \
		if unlikely(EMU86_MODRM_ISREG(mod.mi_type))                   \
			goto err_ilop;                                            \
