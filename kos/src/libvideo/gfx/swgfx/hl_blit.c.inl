...
LOCAL_libvideo_swblitter_blit_rdwrap(LOCAL_struct_video_blitter const *__restrict self,
	...
	if (video_gfx_getflags(src) & VIDEO_GFX_F_XWRAP) {
		...
		if unlikely(OVERFLOW_UADD((video_coord_t)dst_x, size_x, &dst_endx) ||
		            dst_endx > dst->vg_clip.vgc_cxdim) {
			if unlikely((video_coord_t)dst_x >= dst->vg_clip.vgc_cxdim)
				return;
			size_x = dst->vg_clip.vgc_cxdim - (video_coord_t)dst_x;
		}
#ifdef LOCAL_USE_SWBLITTER3
		if unlikely(out_x < 0) {
		...
		if unlikely(OVERFLOW_UADD((video_coord_t)out_x, size_x, &dst_endx) ||
		            dst_endx > out->vg_clip.vgc_cxdim) {
			if unlikely((video_coord_t)out_x >= out->vg_clip.vgc_cxdim)
				return;
			size_x = out->vg_clip.vgc_cxdim - (video_coord_t)out_x;
		}
#endif /* LOCAL_USE_SWBLITTER3 */
		src_x     = wrap(src_x, src->vg_clip.vgc_cxdim);
		src_maxsx = src->vg_clip.vgc_cxdim - (video_coord_t)src_x;
	}
	...
	if (video_gfx_getflags(src) & VIDEO_GFX_F_YWRAP) {
		...
		if unlikely(OVERFLOW_UADD((video_coord_t)dst_y, size_y, &dst_endy) ||
		            dst_endy > dst->vg_clip.vgc_cydim) {
			if unlikely((video_coord_t)dst_y >= dst->vg_clip.vgc_cydim)
				return;
			size_y = dst->vg_clip.vgc_cydim - (video_coord_t)dst_y;
		}
#ifdef LOCAL_USE_SWBLITTER3
		if unlikely(out_y < 0) {
		...
		if unlikely(OVERFLOW_UADD((video_coord_t)out_y, size_y, &dst_endy) ||
		            dst_endy > out->vg_clip.vgc_cydim) {
			if unlikely((video_coord_t)out_y >= out->vg_clip.vgc_cydim)
				return;
			size_y = out->vg_clip.vgc_cydim - (video_coord_t)out_y;
		}
#endif /* LOCAL_USE_SWBLITTER3 */
		src_y     = wrap(src_y, src->vg_clip.vgc_cydim);
		src_maxsy = src->vg_clip.vgc_cydim - (video_coord_t)src_y;
	}
...
#define LOCAL_dst_y_add(v) dst_y += v
	...
	if ((size_x > src_maxsx) && (size_y > src_maxsy)) {
		...
		if ((size_x > src->vg_clip.vgc_cxdim || size_y > src->vg_clip.vgc_cydim) &&
		...
		if unlikely(src_maxsy < src->vg_clip.vgc_cydim) {
			/* Top-left corner */
			...
			while (iter_size_x > src->vg_clip.vgc_cxdim) {
				LOCAL_blit(iter_dst_x, iter_dst_y, 0, src_y, src->vg_clip.vgc_cxdim, src_maxsy);
				iter_size_x -= src->vg_clip.vgc_cxdim;
				iter_dst_x += src->vg_clip.vgc_cxdim;
			}
		...
		/* Iterate whole rows */
		while (iter_size_y >= src->vg_clip.vgc_cydim) {
			/* Left side */
			LOCAL_blit(dst_x, iter_dst_y, src_x, 0, src_maxsx, src->vg_clip.vgc_cydim);

			/* Center */
			iter_size_x = size_x - src_maxsx;
			iter_dst_x = dst_x + src_maxsx;
			while (iter_size_x > src->vg_clip.vgc_cxdim) {
				LOCAL_blit(iter_dst_x, iter_dst_y, 0, 0, src->vg_clip.vgc_cxdim, src->vg_clip.vgc_cydim);
				iter_size_x -= src->vg_clip.vgc_cxdim;
				iter_dst_x += src->vg_clip.vgc_cxdim;
			}

			/* Right side */
			LOCAL_blit(iter_dst_x, iter_dst_y, 0, 0, iter_size_x, src->vg_clip.vgc_cydim);

			iter_size_y -= src->vg_clip.vgc_cydim;
			iter_dst_y += src->vg_clip.vgc_cydim;
		}
		...
		iter_dst_x = dst_x + src_maxsx;
		while (iter_size_x > src->vg_clip.vgc_cxdim) {
			LOCAL_blit(iter_dst_x, iter_dst_y, 0, 0, src->vg_clip.vgc_cxdim, iter_size_y);
			iter_size_x -= src->vg_clip.vgc_cxdim;
			iter_dst_x += src->vg_clip.vgc_cxdim;
		}
		...
		while (size_x > src->vg_clip.vgc_cxdim) {
			LOCAL_blit2dst(0, src_y, src->vg_clip.vgc_cxdim, size_y);
			size_x -= src->vg_clip.vgc_cxdim;
			LOCAL_dst_x_add(src->vg_clip.vgc_cxdim);
		}
		...
		while (size_y > src->vg_clip.vgc_cydim) {
			LOCAL_blit2dst(src_x, 0, size_x, src->vg_clip.vgc_cydim);
			size_y -= src->vg_clip.vgc_cydim;
			LOCAL_dst_y_add(src->vg_clip.vgc_cydim);
		}
...
#define ysrc2dst(y) ((video_coord_t)(((uint64_t)(y) * dst_size_y) / src_size_y))
	...
	if (video_gfx_getflags(src) & VIDEO_GFX_F_XWRAP) {
		...
		if unlikely(OVERFLOW_UADD((video_coord_t)dst_x, dst_size_x, &dst_endx) ||
			...
			if unlikely((video_coord_t)dst_x >= dst->vg_clip.vgc_cxdim)
				return;
			new_dst_size_x = dst->vg_clip.vgc_cxdim - (video_coord_t)dst_x;
		...
		if unlikely(OVERFLOW_UADD((video_coord_t)out_x, dst_size_x, &dst_endx) ||
			...
			if unlikely((video_coord_t)out_x >= out->vg_clip.vgc_cxdim)
				return;
			new_dst_size_x = out->vg_clip.vgc_cxdim - (video_coord_t)out_x;
			src_size_x -= xdst2src(dst_size_x - new_dst_size_x);
			dst_size_x = new_dst_size_x;
		}
#endif /* LOCAL_USE_SWBLITTER3 */
	...
	if (video_gfx_getflags(src) & VIDEO_GFX_F_YWRAP) {
		...
		if unlikely(OVERFLOW_UADD((video_coord_t)dst_y, dst_size_y, &dst_endy) ||
			...
			if unlikely((video_coord_t)dst_y >= dst->vg_clip.vgc_cydim)
				return;
			new_dst_size_y = dst->vg_clip.vgc_cydim - (video_coord_t)dst_y;
		...
		if unlikely(OVERFLOW_UADD((video_coord_t)out_y, dst_size_y, &dst_endy) ||
			...
			if unlikely((video_coord_t)out_y >= out->vg_clip.vgc_cydim)
				return;
			new_dst_size_y = out->vg_clip.vgc_cydim - (video_coord_t)out_y;
			src_size_y -= ydst2src(dst_size_y - new_dst_size_y);
			dst_size_y = new_dst_size_y;
		}
#endif /* LOCAL_USE_SWBLITTER3 */
...
#define LOCAL_dst_y_add(v) dst_y += v
	...
	if ((src_size_x > src_maxsx) && (src_size_y > src_maxsy)) {
		 ...
		 * NOTE: Don't convert the entire "src" buffer; only the rect that will
		 *       be accessed below needs to be copied! */
		if ((src_size_x > src->vg_clip.vgc_cxdim || src_size_y > src->vg_clip.vgc_cydim) &&
		    (video_gfx_getcodec(src) != video_gfx_getcodec(dst) ||
		     video_gfx_getpalette(src) != video_gfx_getpalette(dst))) {
			/* TODO */
		}
		...
		if (iter_src_size_x > src->vg_clip.vgc_cxdim) {
			video_coord_t base_dst_x;
			video_coord_t xwholetiles_i;
			xwholetiles_count = iter_src_size_x / src->vg_clip.vgc_cxdim;
			lasttile_dst_size_x = xsrc2dst(iter_src_size_x % src->vg_clip.vgc_cxdim);
			...
			for (xwholetiles_i = 0; xwholetiles_i < xwholetiles_count; ++xwholetiles_i) {
				...
				LOCAL_stretch(iter_dst_x, dst_y, next_dst_x - iter_dst_x, firsttile_dst_size_y,
				              0, src_y, src->vg_clip.vgc_cxdim, src_maxsy);
				iter_dst_x = next_dst_x;
			}
			iter_src_size_x %= src->vg_clip.vgc_cxdim;
		...
		if likely(iter_src_size_y >= src->vg_clip.vgc_cydim) {
			video_coord_t ywholetiles_i;
			video_dim_t ywholetiles_count = iter_src_size_y / src->vg_clip.vgc_cydim;
			video_dim_t lasttile_dst_size_y = ysrc2dst(iter_src_size_y % src->vg_clip.vgc_cydim);
			...
			for (ywholetiles_i = 0; ywholetiles_i < ywholetiles_count; ++ywholetiles_i) {
				...
				LOCAL_stretch(dst_x, iter_dst_y, firsttile_dst_size_x, tile_h,
				              src_x, 0, src_maxsx, src->vg_clip.vgc_cydim);
				...
				if (xwholetiles_count) {
					...
					do {
						...
						LOCAL_stretch(iter_dst_x, iter_dst_y, next_dst_x - iter_dst_x, tile_h,
						              0, 0, src->vg_clip.vgc_cxdim, src->vg_clip.vgc_cydim);
						iter_dst_x = next_dst_x;
				...
				LOCAL_stretch(iter_dst_x, iter_dst_y, lasttile_dst_size_x, tile_h,
				              0, 0, iter_src_size_x, src->vg_clip.vgc_cydim);

				iter_dst_y = next_dst_y;
			}
			iter_src_size_y %= src->vg_clip.vgc_cydim;
		...
		if (iter_src_size_x > src->vg_clip.vgc_cxdim) {
			...
			for (xwholetiles_i = 0; xwholetiles_i < xwholetiles_count; ++xwholetiles_i) {
				...
				LOCAL_stretch(iter_dst_x, iter_dst_y, next_dst_x - iter_dst_x, iter_dst_size_y,
				              0, 0, src->vg_clip.vgc_cxdim, iter_src_size_y);
				iter_dst_x = next_dst_x;
			}
			iter_src_size_x %= src->vg_clip.vgc_cxdim;
			iter_dst_size_x = lasttile_dst_size_x;
		...
		if (src_size_x > src->vg_clip.vgc_cxdim) {
			/* Whole tiles */
			video_coord_t xwholetiles_i;
			video_dim_t xwholetiles_count = src_size_x / src->vg_clip.vgc_cxdim;
			video_dim_t lasttile_dst_size_x = xsrc2dst(src_size_x % src->vg_clip.vgc_cxdim);
			...
			for (xwholetiles_i = 0; xwholetiles_i < xwholetiles_count; ++xwholetiles_i) {
				...
				LOCAL_stretch2dst(next_dst_x - dst_x, dst_size_y,
				                  0, src_y, src->vg_clip.vgc_cxdim, src_size_y);
#ifdef LOCAL_USE_SWBLITTER3
				out_x += (next_dst_x - dst_x);
#endif /* LOCAL_USE_SWBLITTER3 */
				dst_x = next_dst_x;
			}
			src_size_x %= src->vg_clip.vgc_cxdim;
		...
		if (src_size_y > src->vg_clip.vgc_cydim) {
			/* Whole tiles */
			video_coord_t ywholetiles_i;
			video_dim_t ywholetiles_count = src_size_y / src->vg_clip.vgc_cydim;
			video_dim_t lasttile_dst_size_y = ysrc2dst(src_size_y % src->vg_clip.vgc_cydim);
			...
			for (ywholetiles_i = 0; ywholetiles_i < ywholetiles_count; ++ywholetiles_i) {
				...
				LOCAL_stretch2dst(dst_size_x, next_dst_y - dst_y,
				                  src_x, 0, src_size_x, src->vg_clip.vgc_cydim);
#ifdef LOCAL_USE_SWBLITTER3
				out_y += (next_dst_y - dst_y);
#endif /* LOCAL_USE_SWBLITTER3 */
				dst_y = next_dst_y;
			}
			src_size_y %= src->vg_clip.vgc_cydim;
...
LOCAL_libvideo_swblitter_blit_wrap(LOCAL_struct_video_blitter const *__restrict self,
	...
	if (video_gfx_getflags(dst) & VIDEO_GFX_F_XWRAP) {
		video_coord_t cxend;
		dst_x = wrap(dst_x, dst->vg_clip.vgc_cxdim);
		if (size_x > dst->vg_clip.vgc_cxdim)
			size_x = dst->vg_clip.vgc_cxdim;
		cxend = (video_coord_t)dst_x + size_x;
		if (OVERFLOW_USUB(cxend, dst->vg_clip.vgc_cxdim, &xwrap)) {
			xwrap = 0;
		} else {
			xinb = dst->vg_clip.vgc_cxdim - (video_coord_t)dst_x;
		}
	}
	if (video_gfx_getflags(dst) & VIDEO_GFX_F_YWRAP) {
		video_coord_t cyend;
		dst_y = wrap(dst_y, dst->vg_clip.vgc_cydim);
		if (size_y > dst->vg_clip.vgc_cydim)
			size_y = dst->vg_clip.vgc_cydim;
		cyend = (video_coord_t)dst_y + size_y;
		if (OVERFLOW_USUB(cyend, dst->vg_clip.vgc_cydim, &ywrap)) {
			ywrap = 0;
		} else {
			yinb = dst->vg_clip.vgc_cydim - (video_coord_t)dst_y;
		}
	}
#ifdef LOCAL_USE_SWBLITTER3
...
#define ydst2src(y) ((video_coord_t)(((uint64_t)(y) * src_size_y) / dst_size_y))
	...
	if (video_gfx_getflags(dst) & VIDEO_GFX_F_XWRAP) {
		video_coord_t cxend;
		dst_x = wrap(dst_x, dst->vg_clip.vgc_cxdim);
		if (dst_size_x > dst->vg_clip.vgc_cxdim) {
			src_size_x = xdst2src(dst->vg_clip.vgc_cxdim);
			dst_size_x = dst->vg_clip.vgc_cxdim;
		}
	...
	if (video_gfx_getflags(dst) & VIDEO_GFX_F_YWRAP) {
		video_coord_t cyend;
		dst_y = wrap(dst_y, dst->vg_clip.vgc_cydim);
		if (dst_size_y > dst->vg_clip.vgc_cydim) {
			src_size_y = ydst2src(dst->vg_clip.vgc_cydim);
			dst_size_y = dst->vg_clip.vgc_cydim;
		}
...
LOCAL_libvideo_swblitter_blit_rdwrap1(LOCAL_struct_video_blitter const *__restrict self,
	...
	if (video_gfx_getflags(dst) & VIDEO_GFX_F_XWRAP) {
		...
		if unlikely(OVERFLOW_UADD((video_coord_t)src_x, size_x, &src_endx) ||
		            src_endx > src->vg_clip.vgc_cxdim) {
			if unlikely((video_coord_t)src_x >= src->vg_clip.vgc_cxdim)
				return;
			size_x = src->vg_clip.vgc_cxdim - (video_coord_t)src_x;
		}
		if unlikely(out_x < 0) {
			video_dim_t delta = (video_dim_t)-out_x;
			if (OVERFLOW_USUB(size_x, delta, &size_x))
		...
		if unlikely(OVERFLOW_UADD((video_coord_t)out_x, size_x, &src_endx) ||
		            src_endx > out->vg_clip.vgc_cxdim) {
			if unlikely((video_coord_t)out_x >= out->vg_clip.vgc_cxdim)
				return;
			size_x = out->vg_clip.vgc_cxdim - (video_coord_t)out_x;
		}
		dst_x     = wrap(dst_x, dst->vg_clip.vgc_cxdim);
		dst_maxsx = dst->vg_clip.vgc_cxdim - (video_coord_t)dst_x;
	}
	...
	if (video_gfx_getflags(dst) & VIDEO_GFX_F_YWRAP) {
		...
		if unlikely(OVERFLOW_UADD((video_coord_t)src_y, size_y, &src_endy) ||
		            src_endy > src->vg_clip.vgc_cydim) {
			if unlikely((video_coord_t)src_y >= src->vg_clip.vgc_cydim)
				return;
			size_y = src->vg_clip.vgc_cydim - (video_coord_t)src_y;
		}
		if unlikely(out_y < 0) {
			video_dim_t delta = (video_dim_t)-out_y;
			if (OVERFLOW_USUB(size_y, delta, &size_y))
		...
		if unlikely(OVERFLOW_UADD((video_coord_t)out_y, size_y, &src_endy) ||
		            src_endy > out->vg_clip.vgc_cydim) {
			if unlikely((video_coord_t)out_y >= out->vg_clip.vgc_cydim)
				return;
			size_y = out->vg_clip.vgc_cydim - (video_coord_t)out_y;
		}
		dst_y     = wrap(dst_y, dst->vg_clip.vgc_cydim);
		dst_maxsy = dst->vg_clip.vgc_cydim - (video_coord_t)dst_y;
	}
...
#define LOCAL_blit2out(dst_x_, dst_y_, size_x_, size_y_) \
	...
	if ((size_x > dst_maxsx) && (size_y > dst_maxsy)) {
		...
		/* Deal with unaligned tiles near the top */
		if unlikely(dst_maxsy < dst->vg_clip.vgc_cydim) {
			/* Top-left corner */
			...
			while (iter_size_x > dst->vg_clip.vgc_cxdim) {
				LOCAL_blit(iter_src_x, iter_src_y, 0, dst_y, dst->vg_clip.vgc_cxdim, dst_maxsy);
				iter_size_x -= dst->vg_clip.vgc_cxdim;
				iter_src_x += dst->vg_clip.vgc_cxdim;
			}
		...
		/* Iterate whole rows */
		while (iter_size_y >= dst->vg_clip.vgc_cydim) {
			/* Left side */
			LOCAL_blit(src_x, iter_src_y, dst_x, 0, dst_maxsx, dst->vg_clip.vgc_cydim);

			/* Center */
			iter_size_x = size_x - dst_maxsx;
			iter_src_x = src_x + dst_maxsx;
			while (iter_size_x > dst->vg_clip.vgc_cxdim) {
				LOCAL_blit(iter_src_x, iter_src_y, 0, 0, dst->vg_clip.vgc_cxdim, dst->vg_clip.vgc_cydim);
				iter_size_x -= dst->vg_clip.vgc_cxdim;
				iter_src_x += dst->vg_clip.vgc_cxdim;
			}

			/* Right side */
			LOCAL_blit(iter_src_x, iter_src_y, 0, 0, iter_size_x, dst->vg_clip.vgc_cydim);

			iter_size_y -= dst->vg_clip.vgc_cydim;
			iter_src_y += dst->vg_clip.vgc_cydim;
		}
		...
		iter_src_x = src_x + dst_maxsx;
		while (iter_size_x > dst->vg_clip.vgc_cxdim) {
			LOCAL_blit(iter_src_x, iter_src_y, 0, 0, dst->vg_clip.vgc_cxdim, iter_size_y);
			iter_size_x -= dst->vg_clip.vgc_cxdim;
			iter_src_x += dst->vg_clip.vgc_cxdim;
		}
		...
		src_x += dst_maxsx;
		out_x += dst_maxsx;
		while (size_x > dst->vg_clip.vgc_cxdim) {
			LOCAL_blit2out(0, dst_y, dst->vg_clip.vgc_cxdim, size_y);
			size_x -= dst->vg_clip.vgc_cxdim;
			src_x += dst->vg_clip.vgc_cxdim;
			out_x += dst->vg_clip.vgc_cxdim;
		}
		...
		src_y += dst_maxsy;
		out_y += dst_maxsy;
		while (size_y > dst->vg_clip.vgc_cydim) {
			LOCAL_blit2out(dst_x, 0, size_x, dst->vg_clip.vgc_cydim);
			size_y -= dst->vg_clip.vgc_cydim;
			src_y += dst->vg_clip.vgc_cydim;
			out_y += dst->vg_clip.vgc_cydim;
		}
...
#define ysrc2dst(y) ((video_coord_t)(((uint64_t)(y) * dst_size_y) / src_size_y))
	...
	if (video_gfx_getflags(dst) & VIDEO_GFX_F_XWRAP) {
		...
		if unlikely(OVERFLOW_UADD((video_coord_t)out_x, dst_size_x, &out_endx) ||
		            out_endx > out->vg_clip.vgc_cxdim) {
			video_dim_t new_dst_size_x;
			if unlikely((video_coord_t)out_x >= out->vg_clip.vgc_cxdim)
				return;
			new_dst_size_x = out->vg_clip.vgc_cxdim - (video_coord_t)out_x;
	...
	if (video_gfx_getflags(dst) & VIDEO_GFX_F_YWRAP) {
		...
		if unlikely(OVERFLOW_UADD((video_coord_t)out_y, dst_size_y, &out_endy) ||
		            out_endy > out->vg_clip.vgc_cydim) {
			video_dim_t new_dst_size_y;
			if unlikely((video_coord_t)out_y >= out->vg_clip.vgc_cydim)
				return;
			new_dst_size_y = out->vg_clip.vgc_cydim - (video_coord_t)out_y;
...
#define LOCAL_stretch(out_x_, out_y_, dst_x_, dst_y_, dst_size_x_, dst_size_y_, src_x_, src_y_, src_size_x_, src_size_y_) \
	...
	if ((dst_size_x > dst_maxsx) && (dst_size_y > dst_maxsy)) {
		...
		if (iter_dst_size_x > dst->vg_clip.vgc_cxdim) {
			video_coord_t base_src_x;
			video_coord_t xwholetiles_i;
			xwholetiles_count = iter_dst_size_x / dst->vg_clip.vgc_cxdim;
			lasttile_src_size_x = xdst2src(iter_dst_size_x % dst->vg_clip.vgc_cxdim);
			...
			for (xwholetiles_i = 0; xwholetiles_i < xwholetiles_count; ++xwholetiles_i) {
				...
				LOCAL_stretch(iter_out_x, out_y, 0, dst_y, dst->vg_clip.vgc_cxdim, dst_maxsy,
				              iter_src_x, src_y, next_src_x - iter_src_x, firsttile_src_size_y);
				iter_src_x = next_src_x;
				iter_out_x += dst->vg_clip.vgc_cxdim;
			}
			iter_dst_size_x %= dst->vg_clip.vgc_cxdim;
		...
		if likely(iter_dst_size_y >= dst->vg_clip.vgc_cydim) {
			video_coord_t ywholetiles_i;
			video_dim_t ywholetiles_count = iter_dst_size_y / dst->vg_clip.vgc_cydim;
			video_dim_t lasttile_src_size_y = ydst2src(iter_dst_size_y % dst->vg_clip.vgc_cydim);
			...
			for (ywholetiles_i = 0; ywholetiles_i < ywholetiles_count; ++ywholetiles_i) {
				...
				/* Left side */
				LOCAL_stretch(out_x, iter_out_y, dst_x, 0, dst_maxsx, dst->vg_clip.vgc_cydim,
				...
				if (xwholetiles_count) {
					...
					do {
						...
						LOCAL_stretch(iter_out_x, iter_out_y,
						              0, 0, dst->vg_clip.vgc_cxdim, dst->vg_clip.vgc_cydim,
				...
				/* Right side */
				LOCAL_stretch(iter_out_x, iter_out_y, 0, 0, iter_dst_size_x, dst->vg_clip.vgc_cydim,
				              iter_src_x, iter_src_y, lasttile_src_size_x, tile_h);

				iter_src_y = next_src_y;
				iter_out_y += dst->vg_clip.vgc_cydim;
			}
			iter_dst_size_y %= dst->vg_clip.vgc_cydim;
		...
		if (iter_dst_size_x > dst->vg_clip.vgc_cxdim) {
			...
			for (xwholetiles_i = 0; xwholetiles_i < xwholetiles_count; ++xwholetiles_i) {
				video_coord_t next_src_x;
				next_src_x = base_src_x + (wholetiles_src_size_x * (xwholetiles_i + 1)) / xwholetiles_count;
				LOCAL_stretch(iter_out_x, iter_out_y, 0, 0, dst->vg_clip.vgc_cxdim, iter_dst_size_y,
		...
		if (dst_size_x > dst->vg_clip.vgc_cxdim) {
			/* Whole tiles */
			video_coord_t xwholetiles_i;
			video_dim_t xwholetiles_count = dst_size_x / dst->vg_clip.vgc_cxdim;
			video_dim_t lasttile_src_size_x = xdst2src(dst_size_x % dst->vg_clip.vgc_cxdim);
			video_dim_t wholetiles_src_size_x = src_size_x - lasttile_src_size_x;
			video_coord_t base_src_x = src_x;
			for (xwholetiles_i = 0; xwholetiles_i < xwholetiles_count; ++xwholetiles_i) {
				video_coord_t next_src_x;
				next_src_x = base_src_x + (wholetiles_src_size_x * (xwholetiles_i + 1)) / xwholetiles_count;
				LOCAL_stretch(out_x, out_y, 0, dst_y, dst->vg_clip.vgc_cxdim, dst_size_y,
		...
		if (dst_size_y > dst->vg_clip.vgc_cydim) {
			/* Whole tiles */
			video_coord_t ywholetiles_i;
			video_dim_t ywholetiles_count = dst_size_y / dst->vg_clip.vgc_cydim;
			video_dim_t lasttile_src_size_y = ydst2src(dst_size_y % dst->vg_clip.vgc_cydim);
			video_dim_t wholetiles_src_size_y = src_size_y - lasttile_src_size_y;
			video_coord_t base_src_y = src_y;
			for (ywholetiles_i = 0; ywholetiles_i < ywholetiles_count; ++ywholetiles_i) {
				video_coord_t next_src_y;
				next_src_y = base_src_y + (wholetiles_src_size_y * (ywholetiles_i + 1)) / ywholetiles_count;
				LOCAL_stretch(out_x, out_y, dst_x, 0, dst_size_x, dst->vg_clip.vgc_cydim,
...
LOCAL_libvideo_swblitter_blit_wrap1(LOCAL_struct_video_blitter const *__restrict self,
	...
	if (video_gfx_getflags(out) & VIDEO_GFX_F_XWRAP) {
		video_coord_t cxend;
		out_x = wrap(out_x, out->vg_clip.vgc_cxdim);
		if (size_x > out->vg_clip.vgc_cxdim)
			size_x = out->vg_clip.vgc_cxdim;
		cxend = (video_coord_t)out_x + size_x;
		if (OVERFLOW_USUB(cxend, out->vg_clip.vgc_cxdim, &xwrap)) {
			xwrap = 0;
		} else {
			xinb = out->vg_clip.vgc_cxdim - (video_coord_t)out_x;
		}
	}
	if (video_gfx_getflags(out) & VIDEO_GFX_F_YWRAP) {
		video_coord_t cyend;
		out_y = wrap(out_y, out->vg_clip.vgc_cydim);
		if (size_y > out->vg_clip.vgc_cydim)
			size_y = out->vg_clip.vgc_cydim;
		cyend = (video_coord_t)out_y + size_y;
		if (OVERFLOW_USUB(cyend, out->vg_clip.vgc_cydim, &ywrap)) {
			ywrap = 0;
		} else {
			yinb = out->vg_clip.vgc_cydim - (video_coord_t)out_y;
		}
	}
	if (xwrap && ywrap) /* Must do a partial fill at the top-left */
...
#define yout2src(y) ((video_coord_t)(((uint64_t)(y) * src_size_y) / dst_size_y))
	...
	if (video_gfx_getflags(out) & VIDEO_GFX_F_XWRAP) {
		video_coord_t cxend;
		out_x = wrap(out_x, out->vg_clip.vgc_cxdim);
		if (dst_size_x > out->vg_clip.vgc_cxdim) {
			src_size_x = xout2src(out->vg_clip.vgc_cxdim);
			dst_size_x = out->vg_clip.vgc_cxdim;
		}
	...
	if (video_gfx_getflags(out) & VIDEO_GFX_F_YWRAP) {
		video_coord_t cyend;
		out_y = wrap(out_y, out->vg_clip.vgc_cydim);
		if (dst_size_y > out->vg_clip.vgc_cydim) {
			src_size_y = yout2src(out->vg_clip.vgc_cydim);
			dst_size_y = out->vg_clip.vgc_cydim;
		}
