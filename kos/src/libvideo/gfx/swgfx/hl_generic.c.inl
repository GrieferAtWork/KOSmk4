...
#define LOCAL__video_swgfx_x_line_llhh  LOCAL_FUNC(_video_swgfx_x_line_llhh)
#define LOCAL__video_swgfx_x_line_lhhl  LOCAL_FUNC(_video_swgfx_x_line_lhhl)
...
#define LOCAL_video_swgfx_x_line_llhh  LOCAL_FUNC(video_swgfx_x_line_llhh)
#define LOCAL_video_swgfx_x_line_lhhl  LOCAL_FUNC(video_swgfx_x_line_lhhl)
...
LOCAL_libvideo_swgfx_getcolor_mirror(struct video_gfx const *__restrict self,
	...
	if (video_gfx_getflags(self) & VIDEO_GFX_F_XMIRROR)
		x = (self->vg_clip.vgc_cxdim - 1) - x;
	if (video_gfx_getflags(self) & VIDEO_GFX_F_YMIRROR)
		y = (self->vg_clip.vgc_cydim - 1) - y;
...
#define COHSUTH_YMAX   8 /* 1000 */
	x1 += self->vg_clip.vgc_cxoff;
	y1 += self->vg_clip.vgc_cyoff;
	x2 += self->vg_clip.vgc_cxoff;
	y2 += self->vg_clip.vgc_cyoff;
#define COHSUTH_COMPUTEOUTCODE(x, y, result)           \
	...
	if (x1 > x2) {
		...
		LOCAL_video_swgfx_x_line_v(self, (video_coord_t)x1, (video_coord_t)y1,
	...
	if (y2 > y1) {
		...
		LOCAL_video_swgfx_x_line_llhh(self, (video_coord_t)x1, (video_coord_t)y1, sx, sy, color);
	} else if (y2 < y1) {
		video_dim_t sx = (video_dim_t)((video_coord_t)x2 - (video_coord_t)x1) + 1;
		video_dim_t sy = (video_dim_t)((video_coord_t)y1 - (video_coord_t)y2) + 1;
		LOCAL_video_swgfx_x_line_lhhl(self, (video_coord_t)x1, (video_coord_t)y1, sx, sy, color);
	} else {
		video_dim_t length = (video_dim_t)((video_coord_t)x2 - (video_coord_t)x1) + 1;
		LOCAL_video_swgfx_x_line_h(self, (video_coord_t)x1, (video_coord_t)y1, length, color);
	}
...
LOCAL_libvideo_swgfx_line_mirror(struct video_gfx const *__restrict self,
                                 ...
                                 video_color_t color) {
	if (video_gfx_getflags(self) & VIDEO_GFX_F_XMIRROR) {
		x1 = (self->vg_clip.vgc_cxdim - 1) - x1;
		x2 = (self->vg_clip.vgc_cxdim - 1) - x2;
	}
	if (video_gfx_getflags(self) & VIDEO_GFX_F_YMIRROR) {
		y1 = (self->vg_clip.vgc_cydim - 1) - y1;
		y2 = (self->vg_clip.vgc_cydim - 1) - y2;
	}
...
LOCAL_libvideo_swgfx_hline(struct video_gfx const *__restrict self,
	...
	LOCAL_video_swgfx_x_line_h(self, (video_coord_t)x, (video_coord_t)y, length, color);
}
...
LOCAL_libvideo_swgfx_hline_wrap(struct video_gfx const *__restrict self,
	...
	if (video_gfx_getflags(self) & VIDEO_GFX_F_XWRAP) {
		video_coord_t cxend;
		x = wrap(x, self->vg_clip.vgc_cxdim);
		if (OVERFLOW_UADD((video_coord_t)x, length, &cxend) || length >= self->vg_clip.vgc_cxdim) {
			x = 0;
			length = self->vg_clip.vgc_cxdim;
		} else if (cxend > self->vg_clip.vgc_cxdim) {
			LOCAL_libvideo_swgfx_hline(self, 0, y, cxend - self->vg_clip.vgc_cxdim, color);
		}
	}
...
LOCAL_libvideo_swgfx_hline_mirror(struct video_gfx const *__restrict self,
                                  ...
                                  video_dim_t length, video_color_t color) {
	if (video_gfx_getflags(self) & VIDEO_GFX_F_XMIRROR)
		x = (self->vg_clip.vgc_cxdim - length) - x;
	if (video_gfx_getflags(self) & VIDEO_GFX_F_YMIRROR)
		y = (self->vg_clip.vgc_cydim - 1) - y;
...
LOCAL_libvideo_swgfx_vline(struct video_gfx const *__restrict self,
	...
	LOCAL_video_swgfx_x_line_v(self, (video_coord_t)x, (video_coord_t)y, length, color);
}
...
LOCAL_libvideo_swgfx_vline_wrap(struct video_gfx const *__restrict self,
	...
	if (video_gfx_getflags(self) & VIDEO_GFX_F_YWRAP) {
		video_coord_t cyend;
		y = wrap(y, self->vg_clip.vgc_cydim);
		if (OVERFLOW_UADD((video_coord_t)y, length, &cyend) || length >= self->vg_clip.vgc_cydim) {
			y = 0;
			length = self->vg_clip.vgc_cydim;
		} else if (cyend > self->vg_clip.vgc_cydim) {
			LOCAL_libvideo_swgfx_vline(self, x, 0, cyend - self->vg_clip.vgc_cydim, color);
		}
	}
...
LOCAL_libvideo_swgfx_vline_mirror(struct video_gfx const *__restrict self,
	...
	if (video_gfx_getflags(self) & VIDEO_GFX_F_XMIRROR)
		x = (self->vg_clip.vgc_cxdim - 1) - x;
	if (video_gfx_getflags(self) & VIDEO_GFX_F_YMIRROR)
		y = (self->vg_clip.vgc_cydim - length) - y;
...
LOCAL_libvideo_swgfx_fill(struct video_gfx const *__restrict self,
	...
	LOCAL_video_swgfx_x_fill(self, (video_coord_t)x, (video_coord_t)y,
	                         size_x, size_y, color);
}
...
LOCAL_libvideo_swgfx_fillpoly_c(struct video_gfx const *__restrict self,
	...
	crect.vr_xmin = poly->vp_xmin;
	crect.vr_ymin = poly->vp_ymin;
	crect.vr_xdim = poly->vp_xdim;
	crect.vr_ydim = poly->vp_ydim;

	render_xmin = x + poly->vp_xmin;
	render_ymin = y + poly->vp_ymin;
	...
	if unlikely(OVERFLOW_UADD((video_coord_t)render_xmin, poly->vp_xdim, &render_xend) ||
	            render_xend > GFX_BXEND) {
		if unlikely((video_coord_t)render_xmin >= GFX_BXEND)
			return;
		crect.vr_xdim = GFX_BXEND - render_xmin;
		/*render_xend = GFX_BXEND;*/
	}
	if unlikely(OVERFLOW_UADD((video_coord_t)render_ymin, poly->vp_ydim, &render_yend) ||
	            render_yend > GFX_BYEND) {
		if unlikely((video_coord_t)render_ymin >= GFX_BYEND)
			return;
		crect.vr_ydim = GFX_BYEND - render_ymin;
		/*render_yend = GFX_BYEND;*/
	}

	matrix = VIDEO_IMATRIX2D_INIT(1, 0, 0, 1);
	if (video_gfx_getflags(self) & VIDEO_GFX_F_XMIRROR) {
		crect.vr_xmin = poly->vp_xdim - (crect.vr_xmin + crect.vr_xdim);
		x += poly->vp_xdim - 1;
#ifdef DEFINE_libvideo_swgfx_XXX_xyswap
		video_imatrix2d_set(&matrix, 1, 1, -1);
#else /* DEFINE_libvideo_swgfx_XXX_xyswap */
		video_imatrix2d_set(&matrix, 0, 0, -1);
#endif /* !DEFINE_libvideo_swgfx_XXX_xyswap */
	}
	if (video_gfx_getflags(self) & VIDEO_GFX_F_YMIRROR) {
		crect.vr_ymin = poly->vp_ydim - (crect.vr_ymin + crect.vr_ydim);
		y += poly->vp_ydim - 1;
#ifdef DEFINE_libvideo_swgfx_XXX_xyswap
		video_imatrix2d_set(&matrix, 0, 0, -1);
#else /* DEFINE_libvideo_swgfx_XXX_xyswap */
	...
	LOCAL_video_swgfx_x_fillpoly(self, x, y, cpoly, color, matrix, method);
}
...
LOCAL_libvideo_swgfx_fillpoly(struct video_gfx const *__restrict self,
	...
	if unlikely(render_ymin < (video_offset_t)GFX_BYMIN)
		goto do_clamp;
	if unlikely(OVERFLOW_UADD((video_coord_t)render_xmin, poly->vp_xdim, &render_xend) || render_xend > GFX_BXEND)
		goto do_clamp;
	if unlikely(OVERFLOW_UADD((video_coord_t)render_ymin, poly->vp_ydim, &render_yend) || render_yend > GFX_BYEND)
		goto do_clamp;
	matrix = VIDEO_IMATRIX2D_INIT(1, 0, 0, 1);
	if (video_gfx_getflags(self) & VIDEO_GFX_F_XMIRROR) {
		x += poly->vp_xdim - 1;
	...
	LOCAL_video_swgfx_x_fillpoly(self, x, y, video_polygon_asdata(poly), color, matrix, method);
	return;
do_clamp:
...
LOCAL_libvideo_swgfx_fillpoly_mirror(struct video_gfx const *__restrict self,
	...
	if (video_gfx_getflags(self) & VIDEO_GFX_F_XMIRROR) {
		x += poly->vp_xmin;
		x = (self->vg_clip.vgc_cxdim - poly->vp_xdim) - x;
		x -= poly->vp_xmin;
	}
	if (video_gfx_getflags(self) & VIDEO_GFX_F_YMIRROR) {
		y += poly->vp_ymin;
		y = (self->vg_clip.vgc_cydim - poly->vp_ydim) - y;
		y -= poly->vp_ymin;
	}
...
#define HLINE(x, y, length) LOCAL_video_swgfx_x_line_h(self, x, y, length, color)
#define VLINE(x, y, length) LOCAL_video_swgfx_x_line_v(self, x, y, length, color)
	switch (draw_lines) {
...
LOCAL_libvideo_swgfx_gradient(struct video_gfx const *__restrict self,
	...
	LOCAL_video_swgfx_x_gradient(self, (video_coord_t)x, (video_coord_t)y,
...
LOCAL_libvideo_swgfx_gradient_mirror(struct video_gfx const *__restrict self,
                                     ...
                                     video_color_t const colors[2][2]) {
	video_color_t fixed_colors[2][2];
	switch (video_gfx_getflags(self) & (VIDEO_GFX_F_XMIRROR | VIDEO_GFX_F_YMIRROR)) {
...
LOCAL_libvideo_swgfx_hgradient(struct video_gfx const *__restrict self,
	...
	LOCAL_video_swgfx_x_gradient_h(self, (video_coord_t)x, (video_coord_t)y,
...
LOCAL_libvideo_swgfx_vgradient(struct video_gfx const *__restrict self,
	...
	LOCAL_video_swgfx_x_gradient_v(self, (video_coord_t)x, (video_coord_t)y,
	                               size_x, size_y, locolor, hicolor);
}
...
#undef LOCAL__video_swgfx_x_line_llhh
#undef LOCAL__video_swgfx_x_line_lhhl
...
#undef LOCAL_video_swgfx_x_line_llhh
#undef LOCAL_video_swgfx_x_line_lhhl
