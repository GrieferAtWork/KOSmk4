...
INTDEF ATTR_IN(1) void CC libvideo_gfx_generic__absline_llhh_l(struct video_gfx const *__restrict self, video_coord_t dst_x, video_coord_t dst_y, video_dim_t size_x, video_dim_t size_y, video_color_t color);
INTDEF ATTR_IN(1) void CC libvideo_gfx_generic__absline_lhhl_l(struct video_gfx const *__restrict self, video_coord_t dst_x, video_coord_t dst_y, video_dim_t size_x, video_dim_t size_y, video_color_t color);
...
INTDEF ATTR_RETNONNULL ATTR_INOUT(1) struct video_blitter *FCC libvideo_gfx_generic__blitto(struct video_blitter *__restrict ctx);
...
INTDEF ATTR_RETNONNULL ATTR_INOUT(1) struct video_blitter *FCC libvideo_gfx_noblend__blitto(struct video_blitter *__restrict ctx);
...
INTDEF ATTR_IN(1) ATTR_IN(6) ATTR_IN(7) void CC libvideo_gfx_generic_fillmask_byblit(struct video_gfx const *__restrict self, video_offset_t dst_x, video_offset_t dst_y, video_dim_t size_x, video_dim_t size_y, video_color_t const bg_fg_colors[2], struct video_bitmask const *__restrict bm);
...
INTDEF ATTR_IN(1) ATTR_IN(6) ATTR_IN(9) void CC libvideo_gfx_generic_fillstretchmask_byblit(struct video_gfx const *__restrict self, video_offset_t dst_x, video_offset_t dst_y, video_dim_t dst_size_x, video_dim_t dst_size_y, video_color_t const bg_fg_colors[2], video_dim_t src_size_x, video_dim_t src_size_y, struct video_bitmask const *__restrict bm);
...
libvideo_gfx_generic_update(struct video_gfx *__restrict self, unsigned int what) {
	...
	if (what & VIDEO_GFX_UPDATE_BLEND) {
		...
		case GFX_BLENDMODE_OVERRIDE:
			...
			self->vx_hdr.vxh_blitfrom       = &libvideo_gfx_noblend__blitto;
...
#undef LINK_libvideo_gfx_generic__putcolor_FOO
		...
		self->vx_hdr.vxh_blitfrom         = &libvideo_gfx_generic__blitto;
	...
	if (what & (VIDEO_GFX_UPDATE_FLAGS | VIDEO_GFX_UPDATE_BLEND)) {
		...
		if (GFX_BLENDMODE_GET_MODE(self->vx_blend) == GFX_BLENDMODE_OVERRIDE) {
			if (!(self->vx_flags & VIDEO_GFX_F_LINEAR)) {
				...
				self->_vx_xops.vgfx_absline_lhhl       = &libvideo_gfx_generic__absline_lhhl_l;
			} else {
