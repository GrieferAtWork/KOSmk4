...
struct lockable_buffer_subregion: lockable_buffer {    /* [OVERRIDE(.lbb_data, [0..1][lock(WRITE_ONCE)][NOT(owned)])]
	...
	video_gfx_flag_t                      lbsb_gfx;    /* [const] GFX flags to apply during GFX init */
...
INTDEF WUNUSED ATTR_INOUT(1) ATTR_IN(2) REF struct video_buffer *FCC lockable_buffer__subregion(struct video_buffer *__restrict self, struct video_crect const *__restrict rect, video_gfx_flag_t gfx_flags);
INTDEF WUNUSED ATTR_INOUT(1) ATTR_IN(2) REF struct video_buffer *FCC lockable_buffer_subregion__subregion(struct video_buffer *__restrict self, struct video_crect const *__restrict rect, video_gfx_flag_t gfx_flags);
