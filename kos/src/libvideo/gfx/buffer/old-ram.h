...
#undef CONFIG_HAVE_OLD_RAMBUFFER_PIXELn_FASTPASS
#define CONFIG_HAVE_OLD_RAMBUFFER_PIXELn_FASTPASS

DECL_BEGIN

struct old_video_rambuffer: video_buffer {
	size_t  rb_stride; /* [const] Buffer stride */
	byte_t *rb_data;   /* [1..1][owned][const] Buffer data */
};

struct old_video_rambuffer__for_codec: old_video_rambuffer {
	REF struct video_codec_handle *rbfc_codec; /* [1..1][const] Codec handle */
};

struct old_video_membuffer: old_video_rambuffer {
	void (CC *vm_release_mem)(void *cookie, void *mem);
	void     *vm_release_mem_cookie;
};

struct old_video_membuffer__for_codec: old_video_membuffer {
	REF struct video_codec_handle *vmfc_codec; /* [1..1][const] Codec handle */
};

/* Ram-buffer operator callbacks. */
INTDEF NONNULL((1)) void FCC old_rambuffer_destroy__for_codec(struct video_buffer *__restrict self);
INTDEF NONNULL((1)) void FCC old_rambuffer_destroy(struct video_buffer *__restrict self);
INTDEF ATTR_INOUT(1) ATTR_OUT(2) int FCC old_rambuffer_lock(struct video_buffer *__restrict self, struct video_lock *__restrict lock);
INTDEF ATTR_INOUT(1) NONNULL((2)) int FCC old_rambuffer_lockregion(struct video_buffer *__restrict self, struct video_regionlock *__restrict lock);
INTDEF ATTR_RETNONNULL ATTR_INOUT(1) struct video_gfx *FCC old_rambuffer_initgfx(struct video_gfx *__restrict self);
INTDEF ATTR_RETNONNULL ATTR_INOUT(1) struct video_gfx *FCC old_rambuffer_updategfx(struct video_gfx *__restrict self, unsigned int what);
#define old_rambuffer_rlock       old_rambuffer_lock
#define old_rambuffer_wlock       old_rambuffer_lock
#define old_rambuffer_rlockregion old_rambuffer_lockregion
#define old_rambuffer_wlockregion old_rambuffer_lockregion

#define video_buffer_ops_set_LOCKOPS_like_RAMBUFFER(self)          \
	(void)((self)->vi_rlock        = &old_rambuffer_rlock,         \
	       (self)->vi_wlock        = &old_rambuffer_wlock,         \
	       (self)->vi_unlock       = &libvideo_buffer_noop_unlock, \
	       (self)->vi_rlockregion  = &old_rambuffer_rlockregion,   \
	       (self)->vi_wlockregion  = &old_rambuffer_wlockregion,   \
	       (self)->vi_unlockregion = &libvideo_buffer_noop_unlockregion)
#define video_buffer_ops_set_GFXOPS_like_RAMBUFFER(self)  \
	(void)((self)->vi_initgfx   = &old_rambuffer_initgfx, \
	       (self)->vi_updategfx = &old_rambuffer_updategfx)

INTDEF struct video_buffer_ops old_rambuffer_ops;
INTDEF struct video_buffer_ops old_rambuffer_ops__for_codec;
INTDEF ATTR_RETNONNULL WUNUSED struct video_buffer_ops const *CC _old_rambuffer_ops(void);
INTDEF ATTR_RETNONNULL WUNUSED struct video_buffer_ops const *CC _old_rambuffer_ops__for_codec(void);

INTDEF struct video_buffer_ops old_membuffer_ops;
INTDEF struct video_buffer_ops old_membuffer_ops__for_codec;
INTDEF ATTR_RETNONNULL WUNUSED struct video_buffer_ops *CC _old_membuffer_ops(void);
INTDEF ATTR_RETNONNULL WUNUSED struct video_buffer_ops *CC _old_membuffer_ops__for_codec(void);
...
struct old_gfx_ramdrv: gfx_swdrv {
	byte_t                    *grd_data;   /* [const][== ((struct old_video_rambuffer *):vx_buffer)->rb_data] */
	size_t                     grd_stride; /* [const][== ((struct old_video_rambuffer *):vx_buffer)->rb_stride] */
	struct video_format const *grd_format; /* [const][== &((struct old_video_rambuffer *):vx_buffer)->vb_format] */
};
#define old_gfx_ramdrv_init(self, buffer)            \
	(void)((self)->grd_data   = (buffer)->rb_data,   \
	       (self)->grd_stride = (buffer)->rb_stride, \
	       (self)->grd_format = &(buffer)->vb_format)
#define video_old_ramgfx_getdrv(self) \
	((struct old_gfx_ramdrv *)(self)->_vx_driver)
