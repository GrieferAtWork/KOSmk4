...
VERSION="0.11.1"
SRCPATH="$KOS_ROOT/binutils/src/x/renderproto-$VERSION"
...
if ! [ -f "$SRCPATH/configure" ]; then
	...
	download_file \
		...
		"https://www.x.org/releases/X11R7.7/src/everything/renderproto-$VERSION.tar.gz"
	cmd tar xvf "renderproto-$VERSION.tar.gz"
fi

# Install the PKG_CONFIG file
if ! [ -f "$PKG_CONFIG_PATH/renderproto.pc" ]; then
	cmd mkdir -p "$PKG_CONFIG_PATH"
	cat > "$PKG_CONFIG_PATH/renderproto.pc" <<EOF
prefix=/
exec_prefix=/
