...
SRCPATH="$KOS_ROOT/binutils/src/x/libxkbfile-$VERSION"
OPTPATH="$BINUTILS_SYSROOT/opt/x/libxkbfile-$VERSION"
	...
	if [ "$MODE_FORCE_CONF" == yes ] || ! [ -f "$OPTPATH/Makefile" ]; then
		if ! [ -f "$SRCPATH/configure" ]; then
			...
			download_file \
				...
				"https://www.x.org/releases/X11R7.7/src/everything/libxkbfile-$VERSION.tar.gz"
		...
		(
			...
			cmd bash "../../../../src/x/libxkbfile-$VERSION/configure" \
				--prefix="/" \
				--exec-prefix="/" \
				--bindir="/bin" \
				--sbindir="/bin" \
				...
				--docdir="/usr/share/doc/libxkbfile" \
				--htmldir="/usr/share/doc/libxkbfile" \
				--dvidir="/usr/share/doc/libxkbfile" \
				--pdfdir="/usr/share/doc/libxkbfile" \
				--psdir="/usr/share/doc/libxkbfile" \
...
if ! [ -f "$PKG_CONFIG_PATH/xkbfile.pc" ]; then
	cmd mkdir -p "$PKG_CONFIG_PATH"
	cat > "$PKG_CONFIG_PATH/xkbfile.pc" <<EOF
prefix=/
exec_prefix=/
