...
VERSION="1.2.2"
SRCPATH="$KOS_ROOT/binutils/src/x/scrnsaverproto-$VERSION"
...
if ! [ -f "$SRCPATH/configure" ]; then
	...
	download_file \
		...
		"https://www.x.org/releases/X11R7.7/src/everything/scrnsaverproto-$VERSION.tar.gz"
	cmd tar xvf "scrnsaverproto-$VERSION.tar.gz"
fi

# Install the PKG_CONFIG file
if ! [ -f "$PKG_CONFIG_PATH/scrnsaverproto.pc" ]; then
	cmd mkdir -p "$PKG_CONFIG_PATH"
	cat > "$PKG_CONFIG_PATH/scrnsaverproto.pc" <<EOF
prefix=/
exec_prefix=/
