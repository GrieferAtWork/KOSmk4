...
VERSION="2.1"
SRCPATH="$KOS_ROOT/binutils/src/x/xf86dgaproto-$VERSION"
...
if ! [ -f "$SRCPATH/configure" ]; then
	...
	download_file \
		...
		"https://www.x.org/releases/X11R7.7/src/everything/xf86dgaproto-$VERSION.tar.gz"
	cmd tar xvf "xf86dgaproto-$VERSION.tar.gz"
fi

# Install the PKG_CONFIG file
if ! [ -f "$PKG_CONFIG_PATH/xf86dgaproto.pc" ]; then
	cmd mkdir -p "$PKG_CONFIG_PATH"
	cat > "$PKG_CONFIG_PATH/xf86dgaproto.pc" <<EOF
prefix=/
exec_prefix=/
