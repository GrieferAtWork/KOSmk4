...
VERSION="2.2"
SRCPATH="$KOS_ROOT/binutils/src/x/inputproto-$VERSION"
...
if ! [ -f "$SRCPATH/configure" ]; then
	...
	download_file \
		...
		"https://www.x.org/releases/X11R7.7/src/everything/inputproto-$VERSION.tar.gz"
	cmd tar xvf "inputproto-$VERSION.tar.gz"
fi

# Install the PKG_CONFIG file
if ! [ -f "$PKG_CONFIG_PATH/inputproto.pc" ]; then
	cmd mkdir -p "$PKG_CONFIG_PATH"
	cat > "$PKG_CONFIG_PATH/inputproto.pc" <<EOF
prefix=/
exec_prefix=/
