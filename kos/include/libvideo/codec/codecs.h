...
struct video_codec {
	...
	__ATTR_NONNULL((3))
	void (LIBVIDEO_CODEC_CC *vc_rambuffer_requirements)(__size_t size_x, __size_t size_y,
	                                                    struct video_rambuffer_requirements *__restrict result);

	/* Get a pixel (The caller must ensure that the given x is in-bounds) */
	__ATTR_WUNUSED __ATTR_NONNULL((1))
	video_pixel_t (__ATTR_PURE_P LIBVIDEO_CODEC_CC *vc_getpixel)(__byte_t const *__restrict line, __uintptr_t x);

	/* Set a pixel (The caller must ensure that the given x is in-bounds) */
	__ATTR_NONNULL((1))
	void (LIBVIDEO_CODEC_CC *vc_setpixel)(__byte_t *__restrict line, __uintptr_t x, video_pixel_t pixel);

	/* Copy `num_pixels' neighboring (the caller must ensure that all coords are in-bounds) */
	__ATTR_NONNULL((1, 3))
	void (LIBVIDEO_CODEC_CC *vc_linecopy)(__byte_t *__restrict dst_line, __uintptr_t dst_x,
	...
	__ATTR_NONNULL((1))
	void (LIBVIDEO_CODEC_CC *vc_linefill)(__byte_t *__restrict line, __uintptr_t dst_x,
	                                      video_pixel_t pixel, __size_t num_pixels);

	/* Convert between color and pixel values. */
	__ATTR_WUNUSED __ATTR_NONNULL((1))
	video_color_t (__ATTR_PURE_P LIBVIDEO_CODEC_CC *vc_pixel2color)(struct video_format const *__restrict self, video_pixel_t pixel);
	__ATTR_WUNUSED __ATTR_NONNULL((1))
	video_pixel_t (__ATTR_PURE_P LIBVIDEO_CODEC_CC *vc_color2pixel)(struct video_format const *__restrict self, video_color_t color);
};
