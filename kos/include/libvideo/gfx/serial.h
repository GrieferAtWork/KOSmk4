...
struct video_buffer_fdinfo {
	 ...
	 * >> byte_t *base = mmap(NULL, vbfdi_total, MAP_FILE | MAP_SHARED, vbfdi_fd, 0);
	 * >> byte_t *line = (base + vbfdi_bxoff) + (<Y> * vbfdi_stride);
	 * >> video_pixel_t pixel = (*video_buffer_getcodec(:buffer)->vc_getpixel)(line, vbfdi_pxoff + <X>); */
	...
	__size_t      vbfdi_total;   /* Total # of bytes that should be mapped, starting at `vbfdi_bxoff' */
	video_coord_t vbfdi_pxoff;   /* Remaining pixel-offset before pixel data starts in `vbfdi_fd' (after `vbfdi_bxoff') */
