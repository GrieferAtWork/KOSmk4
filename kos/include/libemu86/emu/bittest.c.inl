...
#define DEFINE_BIT_TEST_AND_op(pred_oldval_mask)                                           \
	IF_64BIT(if (IS_64BIT()) {                                                             \
		...
		if (EMU86_MODRM_ISREG(modrm.mi_type)) {                                            \
			...
			for (;;) {                                                                     \
				oldval = EMU86_EMULATE_READL(addr);                                        \
				if (EMU86_EMULATE_ATOMIC_CMPXCH_OR_WRITEQ(addr, oldval,                    \
		...
		if (EMU86_MODRM_ISREG(modrm.mi_type)) {                                            \
			...
			for (;;) {                                                                     \
				oldval = EMU86_EMULATE_READL(addr);                                        \
				if (EMU86_EMULATE_ATOMIC_CMPXCH_OR_WRITEL(addr, oldval,                    \
				                                          pred_oldval_mask(oldval, mask),  \
				                                          (op_flags & EMU86_F_LOCK) != 0)) \
					break;                                                                 \
				EMU86_EMULATE_LOOPHINT();                                                  \
		...
		if (EMU86_MODRM_ISREG(modrm.mi_type)) {                                            \
			...
			for (;;) {                                                                     \
				oldval = EMU86_EMULATE_READW(addr);                                        \
				if (EMU86_EMULATE_ATOMIC_CMPXCH_OR_WRITEW(addr, oldval,                    \
				                                          pred_oldval_mask(oldval, mask),  \
				                                          (op_flags & EMU86_F_LOCK) != 0)) \
					break;                                                                 \
				EMU86_EMULATE_LOOPHINT();                                                  \
