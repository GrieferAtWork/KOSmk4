...
LIBEMU86_IMPL __ATTR_UNUSED __ATTR_WUNUSED __ATTR_RETNONNULL __ATTR_NONNULL((1, 2)) __byte_t const *
...
#endif /* CONFIG_LIBEMU86_WANT_16BIT */
	{
		...
		case 0x0 << EMU86_MODRM_MOD_SHIFT:
			...
			if (result->mi_rm == EMU86_R_EBP) {
				result->mi_rm     = 0xff;
				result->mi_index  = 0xff;
				result->mi_offset = (__int32_t)__hybrid_unaligned_getle32((__uint32_t *)pc);
				pc += 4;
#if CONFIG_LIBEMU86_WANT_64BIT
