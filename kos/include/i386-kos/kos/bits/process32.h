...
#define process_rtld_elf32_filename(x) \
	((char *)&(x)->pr_phdr[(x)->pr_pnum])
#define process_rtld_elf32_entry(x) \
	(*(__uint32_t *)((__uint32_t)(__libc_strend(process_rtld_elf_filename(x)) + 1 + 3) & ~3))
...
exec_initialize_entry32(struct icpustate *__restrict user_state,
	...
	gpregs_setpbp(&user_state->ics_gpregs, (__uintptr_t)peb_address); /* EXEC_386_PEB_REGISTER */
...
exec_initialize_elf_rtld32(struct icpustate *__restrict user_state,
	...
	user_state_sp -= 4;
	*(__uint32_t *)user_state_sp = (__uint32_t)icpustate_getpc(user_state); /* dl_data->pr_entry */
	...
	__libc_memcpyc(dl_data->pr_phdr, phdr_vec, phdr_cnt, sizeof(Elf32_Phdr));
	dl_data->pr_rtldaddr = (__uint32_t)linker_loadaddr;
	dl_data->pr_pnum     = phdr_cnt;
	gpregs_setpcx(&user_state->ics_gpregs, (__uintptr_t)dl_data);              /* EXEC_X86_64_DL_RTLDDATA_REGISTER */
	gpregs_setpdx(&user_state->ics_gpregs, (__uintptr_t)application_loadaddr); /* EXEC_X86_64_DL_LOADADDR_REGISTER */
