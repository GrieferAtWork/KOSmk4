...
__ASM_L(.macro pushq_seg_cfi_r reg:req, clobber:req)
...
__ASM_L(	.error 'pushq_seg_cfi_r cannot be used with \reg (expected one of %ss, %cs, %ds, %es, %fs.base, %gs.base)')
...
__ASM_L(.macro popq_seg_cfi_r reg:req, clobber:req)
__ASM_L(.ifc __ASM_ARG(\reg),%fs.base)
...
__ASM_L(.ifc __ASM_ARG(\reg),%ds;.else)
__ASM_L(.ifc __ASM_ARG(\reg),%es;.else)
__ASM_L(	.error 'popq_seg_cfi_r cannot be used with \reg (expected one of %ss, %cs, %ds, %es, %fs.base, %gs.base)')
__ASM_L(.endif;.endif;.endif;.endif)
__ASM_L(	popq   __ASM_ARG(\clobber))
__ASM_L(	.cfi_adjust_cfa_offset -8)
