...
__NOTHROW_CB_NCX(__LIBCCALL __LIBC_LOCAL_NAME(psignal))(__signo_t __signo, char const *__s) {
	...
	if (__signam) {
		(__NAMESPACE_LOCAL_SYM __localdep_fprintf_unlocked)(__LOCAL_stderr, "SIG%s\n", __signam);
	} else {
		(__NAMESPACE_LOCAL_SYM __localdep_fprintf_unlocked)(__LOCAL_stderr, "Unknown signal %d\n", __signo);
	}
#if (defined(__CRT_HAVE_flockfile) || defined(__CRT_HAVE__lock_file) || defined(__CRT_HAVE__IO_flockfile)) && (defined(__CRT_HAVE_funlockfile) || defined(__CRT_HAVE__unlock_file) || defined(__CRT_HAVE__IO_funlockfile))
