...
__NAMESPACE_LOCAL_BEGIN
__LOCAL_LIBC(fread) __ATTR_WUNUSED __ATTR_INOUT(4) __ATTR_OUT_OPT(1) __SIZE_TYPE__
__NOTHROW_CB_NCX(__LIBCCALL __LIBC_LOCAL_NAME(fread))(void *__restrict __buf, __SIZE_TYPE__ __elemsize, __SIZE_TYPE__ __elemcount, __FILE *__restrict __stream) {
	__SIZE_TYPE__ __i, __result = 0;
	for (; __elemcount; --__elemcount, ++__result) {
		for (__i = 0; __i < __elemsize; ++__i) {
			int __byte;
			__byte = (__NAMESPACE_LOCAL_SYM __localdep_fgetc)(__stream);
			if (__byte == __EOF)
				goto __done;
