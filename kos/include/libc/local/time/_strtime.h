...
__NAMESPACE_LOCAL_BEGIN
__LOCAL_LIBC(_strtime) __ATTR_RETNONNULL __ATTR_NONNULL((1)) char *
__NOTHROW_NCX(__LIBCCALL __LIBC_LOCAL_NAME(_strtime))(char __buf[9]) {
	...
	__buf[0] = __LOCAL_itoa_decimal(__pn->tm_hour / 10);
	__buf[1] = __LOCAL_itoa_decimal(__pn->tm_hour % 10);
	__buf[2] = ':';
	__buf[3] = __LOCAL_itoa_decimal(__pn->tm_min / 10);
	__buf[4] = __LOCAL_itoa_decimal(__pn->tm_min % 10);
	__buf[5] = ':';
	__buf[6] = __LOCAL_itoa_decimal(__pn->tm_sec / 10);
	__buf[7] = __LOCAL_itoa_decimal(__pn->tm_sec % 10);
	__buf[8] = '\0';
	return 0;
}
__NAMESPACE_LOCAL_END
