 ...
 */
#ifndef __local__W_Getmonths_defined
#define __local__W_Getmonths_defined
#include <__crt.h>
...
__NAMESPACE_LOCAL_BEGIN
__LOCAL_LIBC(_W_Getmonths) __ATTR_WUNUSED __WCHAR_TYPE__ *
__NOTHROW_NCX(__LIBCCALL __LIBC_LOCAL_NAME(_W_Getmonths))(void) {
	/* Essentially, we do:
	...
	for (__i = 0; __i < 12; ++__i) {
		char const *__p;
		__len += 2; /* 2x ':' */
		if ((__p = (__NAMESPACE_LOCAL_SYM __localdep_nl_langinfo)(_NL_TIME_ABMON_1 + __i)) != __NULLPTR)
			__len += (__NAMESPACE_LOCAL_SYM __localdep_strlen)(__p);
		if ((__p = (__NAMESPACE_LOCAL_SYM __localdep_nl_langinfo)(_NL_TIME_MON_1 + __i)) != __NULLPTR)
			__len += (__NAMESPACE_LOCAL_SYM __localdep_strlen)(__p);
	}
	...
	if __likely(__result) {
		...
		for (__i = 0; __i < 12; ++__i) {
			...
			for (__id = 0; __id < 2; ++__id) {
				char const *__p;
				*__dst++ = ':';
				if ((__p = (__NAMESPACE_LOCAL_SYM __localdep_nl_langinfo)(__id ? (_NL_TIME_MON_1 + __i)
				                        : (_NL_TIME_ABMON_1 + __i))) != __NULLPTR) {
					while (*__p)
...
__NAMESPACE_LOCAL_END
#ifndef __local___localdep__W_Getmonths_defined
#define __local___localdep__W_Getmonths_defined
#define __localdep__W_Getmonths __LIBC_LOCAL_NAME(_W_Getmonths)
#endif /* !__local___localdep__W_Getmonths_defined */
...
#undef __local__W_Getmonths_defined
...
#endif /* !__local__W_Getmonths_defined */
