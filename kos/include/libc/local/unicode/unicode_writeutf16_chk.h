...
#define __local_unicode_writeutf16_chk_defined
#include <__crt.h>
#include <hybrid/typecore.h>
__NAMESPACE_LOCAL_BEGIN
__LOCAL_LIBC(unicode_writeutf16_chk) __ATTR_WUNUSED __ATTR_OUT(1) __CHAR16_TYPE__ *
__NOTHROW_NCX(__LIBCCALL __LIBC_LOCAL_NAME(unicode_writeutf16_chk))(__CHAR16_TYPE__ *__restrict __dst, __CHAR32_TYPE__ __ch) {
	if __unlikely(__ch > 0x10ffff)
		return __NULLPTR;
	if __likely(__ch <= 0xffff) {
		if __unlikely(__ch >= 0xd800 && __ch <= 0xdfff)
			return __NULLPTR;
...
#define __local___localdep_unicode_writeutf16_chk_defined
