...
__NAMESPACE_LOCAL_BEGIN
__LOCAL_LIBC(_obstack_begin) __ATTR_ACCESS_WR(1) __ATTR_NONNULL((4, 5)) int
__NOTHROW_NCX(__LIBCCALL __LIBC_LOCAL_NAME(_obstack_begin))(struct obstack *__self, __SIZE_TYPE__ __min_chunk_size, __SIZE_TYPE__ __min_object_alignment, void *(__LIBCCALL *__ob_malloc)(__SIZE_TYPE__ __num_bytes), void (__LIBCCALL *__ob_free)(void *__ptr)) {
	...
	__self->chunkfun.plain = (void *(__LIBCCALL *)(__SIZE_TYPE__))__ob_malloc;
	__self->freefun.plain  = (void (__LIBCCALL *)(void *))__ob_free;
	__self->chunk_size       = __min_chunk_size;
	__self->alignment_mask   = __min_object_alignment - 1;

	__nchunk = (struct _obstack_chunk *)((*__ob_malloc)(__min_chunk_size));
	if (!__nchunk) {
	...
	__self->use_extra_arg      = 0;
	__self->maybe_empty_object = 0;
	__self->alloc_failed       = 0;
	return 1;
}
__NAMESPACE_LOCAL_END
#ifndef __local___localdep__obstack_begin_defined
