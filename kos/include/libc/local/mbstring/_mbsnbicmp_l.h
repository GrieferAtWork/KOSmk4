 ...
 */
#ifndef __local__mbsnbicmp_l_defined
#define __local__mbsnbicmp_l_defined
#include <__crt.h>
...
__LOCAL_LIBC(_mbsnbicmp_l) __ATTR_PURE __ATTR_WUNUSED __ATTR_ACCESS_ROS(1, 3) __ATTR_ACCESS_ROS(2, 3) __ATTR_NONNULL((1, 2)) int
__NOTHROW_NCX(__LIBCCALL __LIBC_LOCAL_NAME(_mbsnbicmp_l))(unsigned char const *__lhs, unsigned char const *__rhs, __SIZE_TYPE__ __max_bytes, __locale_t __locale) {
	unsigned char const *__lhs_end = __lhs + __max_bytes;
	unsigned char const *__rhs_end = __rhs + __max_bytes;
	for (;;) {
		__UINT16_TYPE__ __lc = 0, __rc = 0;
		if (__lhs < __lhs_end) {
			__lc = *__lhs++;
			if ((__NAMESPACE_LOCAL_SYM __localdep__ismbblead_l)(__lc, __locale))
				__lc = (__lhs < __lhs_end && *__lhs) ? (__lc << 8) | *__lhs++ : 0;
		}
		if (__rhs < __rhs_end) {
			__rc = *__rhs++;
			if ((__NAMESPACE_LOCAL_SYM __localdep__ismbblead_l)(__rc, __locale))
				__rc = (__rhs < __rhs_end && *__rhs) ? (__rc << 8) | *__rhs++ : 0;
		}
		if (__lc != __rc) {
...
#undef __local__mbsnbicmp_l_defined
...
#endif /* !__local__mbsnbicmp_l_defined */
