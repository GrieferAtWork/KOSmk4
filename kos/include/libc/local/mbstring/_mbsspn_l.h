 ...
 */
#ifndef __local__mbsspn_l_defined
#define __local__mbsspn_l_defined
#include <__crt.h>
...
__LOCAL_LIBC(_mbsspn_l) __ATTR_PURE __ATTR_WUNUSED __ATTR_ACCESS_RO(1) __ATTR_ACCESS_RO(2) __SIZE_TYPE__
__NOTHROW_NCX(__LIBCCALL __LIBC_LOCAL_NAME(_mbsspn_l))(unsigned char const *__haystack, unsigned char const *__accept, __locale_t __locale) {
	...
	for (;;) {
		...
		if ((__NAMESPACE_LOCAL_SYM __localdep__ismbblead_l)(__ch, __locale))
			__ch = *__next ? (__ch << 8) | *__next++ : 0;
		if (!(__NAMESPACE_LOCAL_SYM __localdep__mbschr_l)(__accept, __ch, __locale))
			break;
		__iter = __next;
	}
