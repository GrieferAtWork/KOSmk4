 ...
 */
#ifndef __local__mbsnbset_l_defined
#define __local__mbsnbset_l_defined
#include <__crt.h>
...
__LOCAL_LIBC(_mbsnbset_l) __ATTR_NONNULL((1)) unsigned char *
__NOTHROW_NCX(__LIBCCALL __LIBC_LOCAL_NAME(_mbsnbset_l))(unsigned char *__buf, unsigned int __ch, __SIZE_TYPE__ __max_bytes, __locale_t __locale) {
	...
	if (!__hib || !(__NAMESPACE_LOCAL_SYM __localdep__ismbblead_l)(__lob, __locale))
		return (unsigned char *)(__NAMESPACE_LOCAL_SYM __localdep_strnset)((char *)__buf, (int)__lob, __max_bytes);
	for (__iter = __buf;;) {
		if (__max_bytes == 0)
			break;
		--__max_bytes;
