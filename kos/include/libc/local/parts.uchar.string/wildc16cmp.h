...
#define __local_wildc16cmp_defined
#include <__crt.h>
__NAMESPACE_LOCAL_BEGIN
__LOCAL_LIBC(wildc16cmp) __ATTR_PURE __ATTR_WUNUSED __ATTR_ACCESS_RO(1) __ATTR_ACCESS_RO(2) int
		...
		if (*__pattern == '*') {
			...
			for (;;) {
				...
				if (__ch == __card_post) {
					...
					if (!(__NAMESPACE_LOCAL_SYM __LIBC_LOCAL_NAME(wildc16cmp))(__string, __pattern))
						return 0;
				} else if (!__ch) {
...
#define __local___localdep_wildc16cmp_defined
