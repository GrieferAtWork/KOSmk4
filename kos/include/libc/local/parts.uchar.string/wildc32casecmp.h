...
#define __local_wildc32casecmp_defined
#include <__crt.h>
...
__LOCAL_LIBC(wildc32casecmp) __ATTR_PURE __ATTR_WUNUSED __ATTR_ACCESS_RO(1) __ATTR_ACCESS_RO(2) int
		...
		if (*__pattern == '*') {
			...
			for (;;) {
				...
				if (__card_post == __ch || __card_post == (__CHAR32_TYPE__)(__NAMESPACE_LOCAL_SYM __localdep_tolower32)((__CHAR32_TYPE__)__ch)) {
					...
					if (!(__NAMESPACE_LOCAL_SYM __LIBC_LOCAL_NAME(wildc32casecmp))(__string, __pattern))
						return 0;
				} else if (!__ch) {
...
#define __local___localdep_wildc32casecmp_defined
