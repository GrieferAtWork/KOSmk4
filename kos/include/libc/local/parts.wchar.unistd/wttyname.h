...
__NOTHROW_RPC(__LIBCCALL __LIBC_LOCAL_NAME(wttyname))(__fd_t __fd) {
	...
	__SIZE_TYPE__ __bufsize = (__NAMESPACE_LOCAL_SYM __localdep_malloc_usable_size)(__ttyname_buf) / sizeof(__WCHAR_TYPE__);
	...
	if (__bufsize < 32) {
		void *__newbuf;
		__bufsize = 32;
		__newbuf  = (__NAMESPACE_LOCAL_SYM __localdep_realloc)(__ttyname_buf, __bufsize * sizeof(__WCHAR_TYPE__));
...
__again:
#endif /* __ERANGE */
	__error = (__NAMESPACE_LOCAL_SYM __localdep_wttyname_r)(__fd, (__WCHAR_TYPE__ *)__ttyname_buf, __bufsize);
	if __likely(__error == 0) {
		/* Trim unused memory (if a certain threshold is exceeded) */
		__SIZE_TYPE__ __retlen = (__NAMESPACE_LOCAL_SYM __localdep_wcslen)((__WCHAR_TYPE__ *)__ttyname_buf) + 1;
		if __likely((__retlen + 32) < __bufsize) {
			void *__retbuf = (__NAMESPACE_LOCAL_SYM __localdep_realloc)(__ttyname_buf, __retlen * sizeof(__WCHAR_TYPE__));
			if __likely(__retbuf)
				__ttyname_buf = __retbuf;
		}
		return (__WCHAR_TYPE__ *)__ttyname_buf;
	}
#ifdef __ERANGE
