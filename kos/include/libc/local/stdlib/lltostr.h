...
/* Dependency: ulltostr from stdlib */
...
__NOTHROW_NCX(__LIBCCALL __LIBC_LOCAL_NAME(lltostr))(__LONGLONG __value, char *__buf) {
	char *__result;
	if (__value < 0) {
		__result    = __localdep_ulltostr((__ULONGLONG)0 - __value, __buf);
		*--__result = '-';
	} else {
		__result = __localdep_ulltostr((__ULONGLONG)__value, __buf);
	}
	return __result;
}
__NAMESPACE_LOCAL_END
