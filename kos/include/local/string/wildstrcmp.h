...
#define __local_wildstrcmp_defined 1
__NAMESPACE_LOCAL_BEGIN
__LOCAL_LIBC(wildstrcmp) __ATTR_WUNUSED __ATTR_PURE __ATTR_NONNULL((1, 2)) int
__NOTHROW_NCX(__LIBCCALL __LIBC_LOCAL_NAME(wildstrcmp))(char const *__pattern,
                                                        char const *__string) {
#line 3518 "kos/src/libc/magic/string.c"
	char __card_post;
	for (;;) {
		if (!*__string) {
		...
		if (*__pattern == '*') {
			...
			for (;;) {
				...
				if (__ch == __card_post) {
					...
					if (!__LIBC_LOCAL_NAME(wildstrcmp)(__string, __pattern))
						return 0;
				} else if (!__ch) {
