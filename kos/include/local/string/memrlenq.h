...
#ifndef ____localdep_memrendq_defined
#define ____localdep_memrendq_defined 1
...
/* Same as `memrchrq', but return `HAYSTACK-1', rather than `NULL' if `NEEDLE' wasn't found. */
__CREDIRECT(__ATTR_RETNONNULL __ATTR_WUNUSED __ATTR_PURE __ATTR_NONNULL((1)),__UINT64_TYPE__ *,__NOTHROW_NCX,__localdep_memrendq,(void const *__restrict __haystack, __UINT64_TYPE__ __qword, __SIZE_TYPE__ __n_qwords),memrendq,(__haystack,__qword,__n_qwords))
#else /* LIBC: memrendq */
#include <local/string/memrendq.h>
/* Same as `memrchrq', but return `HAYSTACK-1', rather than `NULL' if `NEEDLE' wasn't found. */
#define __localdep_memrendq (__NAMESPACE_LOCAL_SYM __LIBC_LOCAL_NAME(memrendq))
#endif /* memrendq... */
#endif /* !____localdep_memrendq_defined */

__NAMESPACE_LOCAL_BEGIN
/* Same as `memrendq', but return the offset from `HAYSTACK', rather than the actual address. */
__LOCAL_LIBC(memrlenq) __ATTR_WUNUSED __ATTR_PURE __ATTR_NONNULL((1)) __SIZE_TYPE__
