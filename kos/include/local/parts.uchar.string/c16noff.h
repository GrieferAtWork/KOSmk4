...
/* Dependency: c16nchrnul from parts.uchar.string */
...
__COMPILER_REDIRECT(__LIBC,__ATTR_PURE __ATTR_RETNONNULL __ATTR_WUNUSED __ATTR_NONNULL((1)),__CHAR16_TYPE__ *,__NOTHROW_NCX,__LIBDCALL,__localdep_c16nchrnul,(__CHAR16_TYPE__ const *__restrict __haystack, __CHAR16_TYPE__ __needle, __SIZE_TYPE__ __maxlen),wcsnchrnul,(__haystack,__needle,__maxlen))
...
__NAMESPACE_LOCAL_BEGIN
/* Same as `wcsnchr', but return `wcsnend(STR, MAX_CHARS)', rather than `NULL' if `NEEDLE' wasn't found. */
#define __localdep_c16nchrnul (*(__CHAR16_TYPE__ *(__LIBDCALL *)(__CHAR16_TYPE__ const *__restrict, __CHAR16_TYPE__, __SIZE_TYPE__))&__LIBC_LOCAL_NAME(wcsnchrnul))
#else /* ... */
__NAMESPACE_LOCAL_END
#include <local/parts.uchar.string/c16nchrnul.h>
__NAMESPACE_LOCAL_BEGIN
/* Same as `wcsnchr', but return `wcsnend(STR, MAX_CHARS)', rather than `NULL' if `NEEDLE' wasn't found. */
#define __localdep_c16nchrnul (*(__CHAR16_TYPE__ *(__LIBDCALL *)(__CHAR16_TYPE__ const *__restrict, __CHAR16_TYPE__, __SIZE_TYPE__))&__LIBC_LOCAL_NAME(c16nchrnul))
#endif /* !... */
...
#define __local___localdep_c16noff_defined 1
