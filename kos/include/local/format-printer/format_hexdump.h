...
#define __local_format_hexdump_defined 1
#include <kos/anno.h>
...
__EXTERNINLINE __ATTR_CONST __ATTR_WUNUSED int __NOTHROW(__LIBCCALL __localdep_isprint)(int __ch) { return __builtin_isprint(__ch); }
	...
	for (;;) {
		...
		if (!(__flags & __FORMAT_HEXDUMP_FNOHEX)) {
			...
			if (__tailspace_count) {
				__temp = __localdep_format_repeat(__printer, __arg, ' ', __tailspace_count);
				if __unlikely(__temp < 0)
					goto __err;
				__result += __temp;
			}
