 ...
 * is smaller than the new block, cannot be extended, and no new block can be allocated,
 * throw an THROW(E_BADALLOC) exception
...
/* Same as `os_heap_realloc()', but only ever extend/truncate a memory block starting at `PTR'.
 * If `NUM_BYTES' is less than or equal to `os_heap_usable_size(PTR)', simply truncate the block
 * and re-return `PTR'. Otherwise, try to allocate memory from within the range
 * `PTR+os_heap_usable_size(PTR) ... PTR+NUM_BYTES-1' and associate that range with the memory
 * block described by `PTR'. If this isn't possible, simply return `NULL'
 ...
 * i.e. `os_heap_usable_size(os_heap_malloc(0))' is allowed to be `0', even though
 *      `os_heap_malloc()' had previously returned a non-NULL value. */
